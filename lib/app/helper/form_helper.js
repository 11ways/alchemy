module.exports = function HawkejsForm(Hawkejs, Blast) {
	 ...
	 */
	var Form = Hawkejs.Helper.extend(function OldFormHelper(view) {
	...
	Form.setMethod(function create(name, options) {
		...
		return this.placeholder(function formResolver(callback) {
			...
			element.classList.add('js-he-form');
			...
			callback(null, element.outerHTML.before('</form>'));
		});
	});
	...
	Form.setMethod(function submit(options) {
		...
		// Create the element
		element = this.view.createElement('button');
		...
		element.innerHTML = '' + (options.label || options.title || '');
	...
	Form.setMethod(function select(_name, _options) {
		...
		element = this.view.createElement('select', options);
		...
		if (typeof options.elements == 'string') {
			...
			for (i = 0; i < elements.length; i++) {
				entry = elements[i];
				op = this.selectOption(entry.key, entry.value, options);

				options_html += op.outerHTML;
			}
		}

		element.classList.add('al-input');
		element.innerHTML = options_html;

		if (this.current_options && this.current_options.field_options) {
			this.current_options.field_options.push({element: element, options: options});
		}

		if (options.wrapper !== false) {
			wrapper = this.view.createElement('div');
	...
	Form.setMethod(function selectOption(key, entry, options) {
		...
		element = this.view.createElement('option', options);
	...
	Form.setMethod(function input(name, options) {
		...
		// Create the element
		element = this.view.createElement(tag_name, options);
		...
		if (options.prepend) {
			...
			wrapper.classList.add('al-has-prepend');
		}

		if (options.append) {
			element.append_html = '<span class="al-append">' + options.append + '</span>';
			wrapper.classList('al-has-append');
		}
		...
		if (data && data.errors.length) {
			element.classList('al-input-error');

			if (wrapper) {
				wrapper.classList.add('al-wrapper-error');
			}
		...
		element.classList.add('al-input-' + name);
		...
		if (wrapper) {
			wrapper.classList.add('al-wrapper-' + name);
			wrapper.appendChild(element);
