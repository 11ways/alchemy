...
module.exports = function alchemyHawkejsHelpers (hawkejs) {
	 ...
	 */
	helpers.form_start = function(modelName, options) {
		...
		if (modelName) {
			...
			if (typeof this.scope.variables[modelName] == 'object'
					&& this.scope.variables[modelName]['_id']) {
				actionType = 'edit';
			} else {
				actionType = 'add';
			}
			...
			if (typeof this.scope.variables[modelName] == 'object') {
				t.form_data = this.scope.variables[modelName];
			}
			
			newAction = '/' + modelName.underscore() + '/' + actionType;
	 ...
	 * @param    {object}    options      Extra options
	 */
	helpers.form_input = function form_input (inputName, options) {
		...
		if (t.form_name) {
			html += ' name="' + t.form_name + '[' + inputName + ']"';
		} else {
			html += ' name="' + inputName + '"';
		}
	 ...
	 * @param    {object}    options      Extra options
	 */
	helpers.form_end = function form_end (buttonName, options) {
	...
	helpers.blueprintField = function blueprintField (name, options) {
		...
		html = hawkejs.helpers.fieldInput(name, {value: options.item[name], return: true});
