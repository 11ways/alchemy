...
Pagination.setMethod(function show(name, options) {
	...
	this.view.print_partial('paginate/navlist', {pages: pages, options: options, wrap: false});
});
...
Pagination.setMethod(function filter(paginationName, fieldName, content) {
	...
	if (url.param('filter_field') == fieldName) {
		...
		config = url.param('filter[' + fieldName + ']');

		if (config) {
			element.setAttribute('value', config);
		}
	}
 ...
 * @version  0.5.0
 */
Pagination.setMethod(function _apply(element) {
	...
	url = Blast.Classes.URL.parse(window.location);
