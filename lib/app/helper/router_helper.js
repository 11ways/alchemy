module.exports = function HawkejsRouter(Hawkejs, Blast) {
	...
	Router.setMethod(function printBreadcrumb() {
		...
		this.view.async(function printBreadcrumbAsync(next) {

			var entries = that.view.internal('breadcrumb_entries'),
			    templates;

			templates = new Hawkejs.Templates(that.view, 'breadcrumb/wrapper');

			console.log('Got entries:', entries);

			that.view.execute(templates, Object.assign({}, that.variables, {entries: entries}), function done(err, result) {
				console.log('Executed breadcrumb wrapper:', err, result);
			});
		});
	});
