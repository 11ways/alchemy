module.exports = function alchemyHelpers(Hawkejs, Blast) {
	...
	Alchemy.setMethod(function getResource(name, data, callback) {
		...
		if (Blast.isNode) {
			...
			conduit.loopback(name, data, callback);
		...
		if (!url && typeof name == 'string') {
			url = hawkejs.scene.helpers.Router.routeUrl('APIResource', {name: name});
		}

		if (!url) {
			return callback(new Error('No URL could be found for route "' + (name.name || name) + '"'));
		}
