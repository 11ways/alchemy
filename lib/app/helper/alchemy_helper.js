module.exports = function alchemyFormHelpers (hawkejs) {
	...
	helpers.errors = function errors() {
		...
		var html = '<div class="alchemy-error" data-error data-age="' + Date.now() + '">';

		if (this.__error__) {
			html += '<pre>' + JSON.stringify(this.__error__, undefined, 2) + '</pre>';
		}
	...
	hawkejs.event.on('ajaxdata', function dataInterceptor(id, variables) {
		...
		if (error) {
			...
			if ($selected) {
				$selected.html('<pre>' + JSON.stringify(error, undefined, 2) + '</pre>');
			} else {
				if (console) {
					console.log(error);
				}
