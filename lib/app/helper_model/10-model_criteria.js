...
Criteria.setMethod(Symbol.asyncIterator, function asyncIterator() {
	...
	// A pre-set skip is the iteration's starting offset, and a pre-set limit
	// bounds the number of iterated records - the iterator used to clobber
	// the skip (always starting at 0) and iterate straight past the limit
	const base_skip = this.options.skip || 0,
	      max_records = this.options.limit || Infinity;
	...
	let context = {
		...
		next  : async function next() {
			...
			if (this.index >= max_records) {
				return {done: true};
			}

			criteria.skip(base_skip + this.index++);
...
Criteria.setMethod(function getCriteriaForAssociation(name, item) {
	...
	// Get the association configuration FIRST (this correctly checks options.associations)
	...
	// Use the modelName from the association config to get the model.
	// This fixes the nested schema alias bug where getAliasModel would fail
	// because it only checks the model's schema, not options.associations.
...
Criteria.setMethod(function sort(value) {
	...
	if (value) {
		...
		// Handle ModelName.field sort patterns
		for (entry of result) {
			if (entry[0].indexOf('.') > -1) {
				let parsed = Criteria.parsePath(entry[0], this);

				if (parsed.association && parsed.association.length) {
					let assoc_name = parsed.association[0];

					// Only store the association if it's actually an association of the model,
					// not just the model's own name
					if (this.model && this.model.associations[assoc_name]) {
						entry[2] = assoc_name;
					}
				}

				entry[0] = parsed.target_path || entry[0];
			}
		}
...
Select.setStatic(function revive(data, criteria) {
	...
	result.query_fields = data.query_fields;
...
Select.setMethod(function parse(path) {
	...
	if (parsed.association) {
		...
		// A Select has no `model` property, so the old `this.model` check
		// here never fired: `select('OwnModel.field')` created a bogus
		// association Select and made the whole find throw at compile time
		let model = this.getModelForAssociations(),
		    name,
		    i;
