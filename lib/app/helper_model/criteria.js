 ...
 * @return   {Object}
 */
Criteria.setStatic(function parsePath(path) {
...
Criteria.setMethod(function toDry() {
	...
	var context = this[AUGMENTED] || this,
	    value = {};

	if (this.model) {
		value.model = this.model.name;
	}
...
Criteria.setMethod(function shouldQueryAssociation(name) {
	...
	if (this.getAssociationsToSelect()) {
		...
		// There are no explici associations, look at the recursive level
...
Criteria.setMethod(function getCriteriaForAssociation(name, item) {
	...
	let association = this.model.getAssociation(name),
	    assoc_crit = assoc_model.find(),
	    assoc_key = association.options.foreignKey,
	    local_key = association.options.localKey,
	    options = this.options,
	    select;

	if (Array.isArray(data[local_key])) {
		assoc_crit.where(assoc_key).in(data[local_key]);
	} else {
		assoc_crit.where(assoc_key).equals(data[local_key]);
	}
...
Select.setMethod(function parse(path) {
	...
	var context,
	    select = this,
	    parsed = Criteria.parsePath(path);
	...
	if (parsed.association) {
		...
		for (i = 0; i < parsed.association.length; i++) {
			select = this.addAssociation(parsed.association[i]);
		}
	}
		...
		if (item.value != null) {
			if (Array.isArray(item.value)) {
				...
				for (i = 0; i < item.value.length; i++) {
					result[i] = field.castCondition(item.value[i]);
				}
