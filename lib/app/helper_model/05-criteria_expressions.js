...
Expression.setProperty(function field() {
	...
	let main_model = this.criteria?.model;
	...
	if (this.association) {
		let info = main_model.getAssociation(this.association);

		if (info) {
			context_model = Model.get(info.model_name);
		}
	} else {
		context_model = main_model;
	}

	if (context_model) {
		return context_model.schema.getField(this.target_path);
	}
});
...
FieldExpression.setMethod(function normalizeAssociationValues() {
	...
	// The criteria has to be passed directly: `{criteria: ...}` is not a
	// recognized find option, so the association conditions were silently
	// dropped and this queried the ENTIRE associated collection
	assoc_crit.setOption('document', false);

	assoc_model.find('all', assoc_crit, function gotAssocItems(err, items) {
		...
		for (i = 0; i < items.length; i++) {
			...
			// Non-document results are wrapped per model name
			record = items[i];

			if (record[assoc_model.name]) {
				record = record[assoc_model.name];
			}

			values.push(record[association.options.foreignKey]);
		}
