...
Document.setMethod(function hasChanged(name) {
	...
	// When we have nothing to compare to, assume yes
	if (!this.$attributes.original_record) {
		return true;
	}
	...
	for (key in this.$attributes.original_record) {
		...
		if (key == '_id') {
			if (String(this.$attributes.original_record._id) == String(this.$attributes.original_record._id)) {
				continue;
			} else {
				return true;
			}
 ...
 * @version  1.0.4
 */
Document.setMethod(function resetFields() {
