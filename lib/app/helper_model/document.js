...
if (Blast.isBrowser) {
	...
	Blast.once('hawkejs_init', function gotScene(hawkejs, variables, settings, view) {
		...
		for (model_name in model_info) {
			...
			for (key in config.associations) {
...
Document.setStatic(function getDocumentClass(model) {
	...
	if (DocClass == null) {
		...
		DocClass.Model = Blast.Classes.Alchemy.Client.Model[model_name];
	}
...
Document.setStatic(function unDry(obj, cloned) {
	...
	if (cloned || Blast.isNode || obj[Blast.Classes.Alchemy.IndexedDb.from_cache_symbol]) {
	...
	if (store_in_cache) {
		...
		if (!window.hawkejs) {
			Blast.setImmediate(function whenHawkejs() {
				if (DocClass.Model.hasServerAction('readDatasource')) {
 ...
 * @return   {Boolean}
 */
Document.setMethod(Blast.alikeSymbol, function alike(other) {

	if (!(other instanceof this.constructor)) {
		return false;
	}

	if (!Object.alike(this.$main, other.$main)) {
		return false;
	}

	return Object.alike(this.$_attributes, other.$_attributes);
});
...
Document.setMethod(function save(data, options, callback) {
	...
	function updateDoc(err, save_result) {
		...
		main._id = alchemy.castObjectId(save_result[that.$model_name]._id);
	...
	if (Blast.isBrowser) {
		if (this.hasChanged()) {
			this.storeInCache({local_save: true});
			set_remote_saved = true;
		} else if (this.$main._$needs_remote_save) {
			set_remote_saved = true;
		}
	}

	if (set_remote_saved) {
		pledge.done(function whenDone(err, result) {

			if (err) {
				return;
			}

			// Mark as also having been saved on the remote server
			that.storeInCache({remote_save: true});
		});
	}
 ...
 * @param    {Function} callback
 */
Document.setMethod(function storeInCache(options, callback) {
	...
	if (!Blast.isBrowser || !this._id) {
		Blast.nextTick(callback);
		return false;
	}

	if (cache_stores_in_progress.has(this._id) && !options.local_save && !options.remote_save) {
		Blast.nextTick(callback);
		return false;
	}
	...
	let data = JSON.toDryObject(JSON.clone(this.$main, 'toDatasource')),
	    time;

	// Add number values to created & updated
	if (data.created) {
		data.created.timestamp = Number(this.created);
	}

	if (data.updated) {
		data.updated.timestamp = Number(this.updated);
	}
	...
	if (window.sessionStorage && this.updated) {
		window.sessionStorage[this._id] = Number(this.updated);
	}

	let that = this;

	this.$model._create(data, function done(err, result) {
