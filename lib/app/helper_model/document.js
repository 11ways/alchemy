...
Document.setMethod(function storeInCache(options, callback) {
	...
	return Function.series(async function getLocalVersion(next) {
		...
		let data = JSON.toDryObject(JSON.clone(that.$main, 'toDatasource')),
		    time;

		// Add number values to created & updated
		if (data.created) {
			data.created.timestamp = Number(that.created);
		}

		if (data.updated) {
			data.updated.timestamp = Number(that.updated);
		}
		...
		if (window.sessionStorage && that.updated) {
			window.sessionStorage[that._id] = Number(that.updated);
		}

		that.$model._create(data, function done(err, result) {
