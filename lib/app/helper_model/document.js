...
/**
 * Read the session-storage updated-stamp of a record, tolerating
 * environments where sessionStorage access itself throws (blocked cookies).
 *
 * @param    {string}   id
 *
 * @return   {string|null}
 */
function getSessionStamp(id) {
	try {
		return (typeof window != 'undefined' && window.sessionStorage) ? window.sessionStorage[id] : null;
	} catch (err) {
		return null;
	}
}
...
/**
 * Write the session-storage updated-stamp of a record.
 * The stamp is only a cache-refresh optimization, so quota or security
 * errors are silently ignored.
 *
 * @param    {string}   id
 * @param    {number}   value
 */
function setSessionStamp(id, value) {
	try {
		if (typeof window != 'undefined' && window.sessionStorage) {
			window.sessionStorage[id] = value;
		}
	} catch (err) {
		// Ignore: worst case the record gets re-cached once more
	}
}
...
Document.setStatic(function unDry(obj, cloned) {
	...
	} else if (result.$main.updated && getSessionStamp(result.$main._id) == Number(result.$main.updated)) {
		store_in_cache = false;
	}
	...
	if (store_in_cache) {
		...
		if (result.$main.updated) {
			setSessionStamp(result.$main._id, Number(result.$main.updated));
		}
...
Document.setMethod(function setDataRecord(record, options) {
	...
	// Never OVERWRITE an existing original_record though: unDry restores it
	// before the constructor runs, and re-snapshotting the (possibly already
	// modified) $main would erase the document's dirty state.
	if (this.hasObjectFields() && !this.$is_cloned && !this.$attributes.original_record) {
...
Document.setMethod(function save(data, options, callback) {
	...
	if (Blast.isBrowser) {
		...
		if (!this[pk_name]) {
			...
			// A copy: writing `create` into the caller's (possibly reused)
			// options object would force CREATE semantics onto the next,
			// unrelated save made with that same object
...
Document.setMethod(function checkAndInformDatasource(options, callback) {
	...
	if (this.$main.updated && getSessionStamp(this.$main._id) == Number(this.$main.updated)) {
		store_in_cache = false;
	}
	...
	if (store_in_cache) {
		...
		if (this.$main.updated) {
			setSessionStamp(this.$main._id, Number(this.$main.updated));
		}
...
Document.setMethod(function informDatasource(options, callback) {
	...
	let series_result = Function.series(async function getLocalVersion(next) {
		...
		// Ask for the RAW cached row: a revived local Document loses the
		// `_$` bookkeeping fields in toApp, so a pending offline save looked
		// unflagged here - and this refresh then re-cached the record
		// WITHOUT its needs-remote-save flag, silently unqueueing the save.
		let local;

		try {
			local = await that.$model.datasource.getUpperVersion(that.$model, that.$pk);
		} catch (err) {
			console.log(err);
			return next();
		}
		...
		if (local && local._$local_save_time && local._$local_save_time > Number(that.updated)) {
			// The cache holds a local edit NEWER than this document:
		...
		if (local && local._$needs_remote_save) {
			// Keep the pending-save flag AND its original edit time: stamping
			// it anew would make a stale queued edit look fresh and beat
			// newer remote versions in conflict resolution.
			options.local_save = local._$local_save_time || true;
		}
