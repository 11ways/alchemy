...
Document.setProperty(function $main() {
	...
	if (!this.$record[this.$model_name]) {
		this.$record[this.$model_name] = {};
	}

	return this.$record[this.$model_name];
	...
	this.$record[this.$model_name] = data;
});
...
Document.setMethod(function setDataRecord(record, options) {
	...
	// do so now
	if (!record[this.$model_name] && !Object.isEmpty(record)) {
		record = {
