...
Criteria.setMethod(Blast.checksumSymbol, function toChecksum() {
	...
	// The target datasource is part of the identity: a Fallback datasource
	// reads the same criteria against both its sub-datasources, and without
	// this discriminator the model's query cache served the LOWER's result
	// for an upper (raw cache) read - which lacks the `_$` bookkeeping
	// fields the offline-sync machinery depends on
	if (this.datasource) {
		result.push(this.datasource.name);
	}
...
Criteria.setMethod(function applyConditions(conditions, clear_field = true) {
	...
	if (keys.length > 1) {
		...
		if (has_group_key) {
			// A multi-key object mixing field conditions with a group key, like
			// `{a: 1, $or: [...]}`, must bind as ONE AND-ed unit. Compiled
			// key-by-key the group attaches to the ENCLOSING group as a sibling
			// of `a` - fatal inside an $or branch, where it widens the match
			// (`a OR ...` instead of `a AND (...)`). The equivalent single-key
			// `{$and: [{a: 1}, {$or: [...]}]}` compiles correctly.
			let split = [],
			    key;

			for (key of keys) {
				split.push({[key]: conditions[key]});
			}

			return context.applyConditions({$and: split}, clear_field);
		}
	}
						...
						if (typeof context[method] != 'function') {
							if (method == 'regex') {
								...
								if (typeof value[key] == 'string') {
									// Mongo treats a string $regex as a
									// PATTERN - escaping it turned every
									// pattern into a literal-text match
									regex = new RegExp(value[key], value.$options || '');
								} else {
								...
								// Consumed by the $regex handler above -
								// when key order put $options first, this
								// used to throw
								continue;
							} else {
...
Criteria.setMethod(function _applyGroup(type, value) {
	...
	// The full path of the active expression: `target_path` alone loses the
	// association part, making `where('Boss.name').equals(a).or(b)` compile
	// its second branch against the ROOT `name` field
	const getActivePath = () => {
		let active = context._active;

		if (active.association) {
			return active.association + '.' + active.target_path;
		}
	...
	if (arguments.length > 1) {
		...
		context = context.where(getActivePath());
