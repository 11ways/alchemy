...
FieldConfig.setMethod(function getModel() {
	...
	if (this.association) {
		try {
			...
			// Association lookup failed - this can happen for invalid/unknown associations
			// Fall through to return the context model instead
			alchemy.distinctProblem(
				'field-config-assoc-' + this.association,
				'FieldConfig failed to resolve association "' + this.association + '": ' + err.message
			);
		}
	}
