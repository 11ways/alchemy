...
Model.setStatic(function hasServerAction(action) {
	...
	if (typeof hawkejs == 'undefined' || !hawkejs.scene) {
		// Not ready yet: report "no server action" instead of throwing.
		// This gets called from idle callbacks (the offline-cache refresh)
		// where a throw is uncaught and the surrounding pledge never settles
		return false;
	}
	...
	pledge = Function.series(false, function doBeforeType(next) {
		...
		that.issueDataEvent('afterData', [criteria, records], function afterFound(err) {
			...
			if (criteria.options.document !== false) {
				...
				// Deliberately also assigns the null of `available: false`
				// finds: null means "count skipped", whereas the length
				// fallback would misreport a full page's length as the total
				records.available = available;
...
Model.setMethod(function createDocument(data, options) {
	...
	if (data && this.constructor.Document.isDocument(data)) {
		...
		// Keep the OTHER $record entries too (populated association data):
		// rebuilding only the main record silently dropped them, so a
		// save() of a document with attached children never cascaded into
...
Model.setMethod(function saveRecord(document, options, callback) {
	...
	pledge = Function.series(function doAudit(next) {
		...
		// A plain emit, NOT issueEvent/issueDataEvent: 'saving' maps to the
		// beforeSave method, which createRecord/updateRecord already invoke
		// post-validation. Issuing the mapped event here too made every
		// model & behaviour beforeSave hook run TWICE per save - the second
		// time without `creating`.
		that.emit('saving', document, options, creating, function afterSavingEvent(err) {
			return next(err);
		});
		...
		Object.each(document.$record, function eachEntry(entry, key) {
			...
			// A plural association holds a list: each child record has to be
			// checked & stamped individually. Setting properties on the LIST
			// object was a no-op, so new children were created without their
			// parent's foreign key, and every unchanged populated child was
			// re-saved as a side effect of saving the parent.
			let records;

			if (Array.isArray(entry)) {
				records = entry;
			} else if (typeof entry.toArray == 'function') {
				records = entry.toArray();
			} else {
				records = [entry];
			}
			...
			for (let record of records) {
				...
				if (typeof record.hasChanged == 'function' && !record.hasChanged()) {
					continue;
				}

				if (foreign_key_value != null) {
					record[assoc.options.foreignKey] = foreign_key_value;
				}

				// Add the task
				tasks.push(function doSave(next) {
					var a_model = that.getModel(assoc.modelName);
					a_model.save(record, next);
				});
			}
		});
...
Model.setMethod(function createRecord(document, options, callback) {
	...
	Function.series(function recompute(next) {
		...
		that.issueDataEvent('beforeSave', [document, options, true], next);
	}, function done(err) {
...
Model.setMethod(function updateRecord(document, options, callback) {
	...
	Function.series(function recompute(next) {
		...
		that.issueDataEvent('beforeSave', [document, options, false], next);
	}, function done(err) {
 ...
 * Records whose remote version changed AFTER the queued local edit was made
 * are dropped (the remote version is adopted locally instead): a stale flag
 * left behind by an old session must never overwrite newer server data.
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    1.0.6
 * @version  1.4.7
...
Model.setMethod(async function getRecordsToBeSavedRemotely() {
	...
	if (!this.datasource.getRecordsToSync) {
		return new Blast.Classes.Alchemy.Client.DocumentList([]);
	}

	let records = await this.datasource.getRecordsToSync(this);

	let result = [];

	for (let record of records) {

		let doc = this.createDocument(record);

		try {
			if (await this.shouldPushLocalSave(doc)) {
				result.push(doc);
			}
		} catch (err) {
			// The remote version could not be checked (usually: still offline).
			// Leave the record flagged and try again on a later sync.
		}
	}

	return new Blast.Classes.Alchemy.Client.DocumentList(result);
}, false);

/**
 * May this locally-flagged record still be pushed to the server?
 * Resolves the conflict when it may not: the remote version is adopted into
 * the local cache (clearing the flag), because the server moved on after the
 * local edit was made.
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    1.4.7
 * @version  1.4.7
 *
 * @param    {Document}   doc   A document built from the flagged cache record
 *
 * @return   {boolean}
 */
Model.setMethod(async function shouldPushLocalSave(doc) {

	let local_time = doc.$main._$local_save_time || doc.$main._$cache_time;

	// A remote-only read: it either answers authoritatively or throws (which
	// the caller treats as "defer to a later sync attempt").
	let remote = await this.find('first', {
		conditions  : {_id: doc.$pk},
		only_remote : true,
	});

	if (!remote) {
		// The record never reached the server: a pending create, push it.
		return true;
	}

	let remote_updated = remote.updated ? Number(remote.updated) : 0;

	if (local_time && remote_updated <= Number(local_time)) {
		// The queued local edit is the newest change: push it.
		return true;
	}

	// The server was updated after this local edit was made (or the local
	// timestamps are missing entirely): the flagged copy is stale. Server wins.
	if (this.datasource.adoptRemoteVersion) {
		await this.datasource.adoptRemoteVersion(this, remote.$main);
	}
