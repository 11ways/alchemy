...
Model.setMethod(function find(type, criteria, callback) {
	...
	if (!Classes.Alchemy.Criteria.Criteria.isCriteria(criteria)) {
		try {
			...
			criteria = new Classes.Alchemy.Criteria();
	...
	pledge = Function.series(false, function doBeforeType(next) {
	...
	}, function normalizeCriteria(next) {
		criteria.normalize().done(next);
		...
		if (!query || (query.recursive <= 0 && !options.contain) || options.contain === true) {
			return next();
		}
...
Model.setMethod(function beforeFindList(criteria, callback) {
	...
	if (!criteria.options.fields.length) {
		criteria.select('_id');
	}
