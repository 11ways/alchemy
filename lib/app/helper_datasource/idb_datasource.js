 ...
 * @version  1.1.0
 */
Idb.setMethod(function collection(name) {
	...
	this._collections.set(name, pledge);

	// A failed open must not poison the cache (this used to be a plain
	// memoize): one transient IndexedDB error (blocked upgrade, private
	// mode quirks) kept that table broken for the whole session
	pledge.done(err => {
		if (err && that._collections.get(name) === pledge) {
			that._collections.delete(name);
		}
	});
...
Idb.setMethod(function _read(context) {
	...
	return Swift.waterfall(
		...
		compiled => {
			...
			// Ask for the wrapped {rows, available} result: the plain
			// row-array resolve dropped `available`, so cache-served
			// paginated finds reported `undefined` as their total
			options.wrap_result = true;
