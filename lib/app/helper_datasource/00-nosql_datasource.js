...
/**
 * Add $lookup and $unwind stages for an association to the aggregate pipeline.
 *
 * OR vs AND Group Handling:
 * -------------------------
 * The handling differs between OR and AND groups due to their different semantics:
 *
 * AND groups (`field = X AND Parent.field = Y`):
 *   - Records MUST have the association to potentially match
 *   - We can filter out null foreign keys BEFORE the $lookup (optimization)
 *   - $unwind can filter out records without associations (no preserveNullAndEmptyArrays)
 *
 * OR groups (`field = X OR Parent.field = Y`):
 *   - Records WITHOUT the association can still match on other branches
 *   - We must NOT filter out null foreign keys before $lookup
 *   - $unwind MUST use preserveNullAndEmptyArrays: true to keep non-matching records
 *   - All lookups are collected first, then conditions are applied together in $or
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    1.4.0
 * @version  1.4.0
 *
 * @param    {Object}   aggregate              The aggregate object with pipeline and lookups
 * @param    {Object}   assoc                  The association configuration
 * @param    {Model}    assoc_model            The associated model
 * @param    {Object}   options                Options for lookup generation
 * @param    {boolean}  options.preserveNull   Use preserveNullAndEmptyArrays on $unwind (for OR groups)
 * @param    {boolean}  options.addNullCheck   Add $match to filter out null foreign keys (for AND groups)
 */
function addLookupForAssociation(aggregate, assoc, assoc_model, options = {}) {

	const { preserveNull = false, addNullCheck = false } = options;

	// For BelongsTo with AND semantics, filter out records with null foreign keys
	if (addNullCheck && assoc.type == 'BelongsTo') {
		let temp = {};
		temp[assoc.options.localKey] = {$ne: null};
		aggregate.pipeline.push({ $match: temp });
	}
	...
	// Add $unwind for singular associations
	if (assoc.options.singular) {
		if (preserveNull) {
			// For OR groups: preserve records that don't have the association
			aggregate.pipeline.push({
				$unwind: {
					path: '$' + assoc.alias,
			...
			// For AND groups: simple $unwind (filters out records without association)
			aggregate.pipeline.push({
				$unwind: '$' + assoc.alias
			});
		}
...
function convertCriteriaGroupToConditions(criteria, group, config, context) {
	...
	// Check if this is an OR group - affects how we handle associations
	let is_or_group = group.group_type === 'or';
	...
	// Cache for nested group results to avoid double-processing
	// Uses the entry object itself as key for robustness (not affected by array index changes)
	let nested_results_cache = new Map();

	// For OR groups with associations, we need a different approach:
	// 1. First, collect all required lookups (with preserveNullAndEmptyArrays)
	// 2. Then apply all conditions together in the $or
	// This is because OR semantics require that records WITHOUT the association
	// can still match on other branches of the OR.
	if (is_or_group) {
		// First pass: collect all required lookups from direct associations and nested groups
		for (i = 0; i < group.items.length; i++) {
			entry = group.items[i];

			// Handle direct association conditions
			if (entry.association && entry.association != criteria.model.model_name) {
				...
				if (!aggregate?.lookups?.[assoc.alias]) {
					...
					if (assoc.type != 'BelongsTo' && assoc.type != 'HasMany') {
						throw new Error('No support for ' + assoc.type + ' association yet');
					}

					// For OR groups: no null check, use preserveNullAndEmptyArrays
					addLookupForAssociation(aggregate, assoc, assoc_model, {
						preserveNull: true,
						addNullCheck: false
					});
				}
			...
			// Handle nested groups - process now and cache result
			if (entry.group_type) {
				let nested_result = convertCriteriaGroupToConditions(entry.criteria, entry, config, context);
				nested_results_cache.set(entry, nested_result);

				// Hoist lookups and $addFields from nested groups
				if (nested_result.pipeline && nested_result.lookups) {
					getAggregate();
					
					for (let alias in nested_result.lookups) {
						if (!aggregate.lookups[alias]) {
							aggregate.lookups[alias] = true;
							
							// Find and add the lookup and unwind stages for this alias
							for (let stage of nested_result.pipeline) {
								if (stage.$lookup && stage.$lookup.as === alias) {
									aggregate.pipeline.push(stage);
								} else if (stage.$unwind) {
									let path = typeof stage.$unwind === 'string' 
										? stage.$unwind 
										: stage.$unwind.path;
									if (path === '$' + alias) {
										// Ensure preserveNullAndEmptyArrays for OR groups
										aggregate.pipeline.push({
											$unwind: {
												path: path,
												preserveNullAndEmptyArrays: true
									...
									// Also hoist $addFields (needed for stringified regex matching)
									aggregate.pipeline.push(stage);
								}
							}
						}
	...
	for (i = 0; i < group.items.length; i++) {
		...
		if (entry.association && !is_or_group) {
			// For AND groups, we can filter before lookups (original behavior)
			getAggregate();
			...
			if (!aggregate.lookups[assoc.alias]) {
				...
				// For AND groups: use null check for BelongsTo, no preserveNullAndEmptyArrays
				addLookupForAssociation(aggregate, assoc, assoc_model, {
					preserveNull: false,
					addNullCheck: assoc.type == 'BelongsTo'
				});
			}
		}

		if (entry.group_type) {
			// Use cached result from first pass if available (for OR groups)
			let compiled_group = nested_results_cache.get(entry)
				|| convertCriteriaGroupToConditions(entry.criteria, entry, config, context);
			let obj = {};

			if (compiled_group.$and && entry.group_type == 'and') {
				// Unwrapping a compiled {$and: [...]} is only safe when it gets
				// rewrapped with the same operator below. An OR entry can also
				// compile to {$and: [...]} (a single-item group collapses to
				// that item), and unwrapping THAT here would rewrap the AND-ed
				// conditions as $or - `a AND b` silently became `a OR b`.
				compiled_group = compiled_group.$and;
			} else if (compiled_group.pipeline) {
				// Nested group contains associations that required $lookup
				// We need to:
				// 1. Hoist the $lookup/$unwind/$addFields stages to the parent aggregate
				// 2. Extract $match conditions and use them in the $or/$and
				getAggregate();

				// Merge lookups from nested group
				Object.assign(aggregate.lookups, compiled_group.lookups);

				// Hoist all non-$match stages to the parent pipeline
				// and collect $match conditions for the $or/$and
				let match_conditions = [];
				for (let stage of compiled_group.pipeline) {
					if (stage.$match) {
						// Extract the match condition
						// If it's wrapped in $and or $or, flatten it appropriately
						let match = stage.$match;
						if (match.$and) {
							match_conditions.push(...match.$and);
						} else if (match.$or) {
							// Preserve $or structure as a single condition
							match_conditions.push(match);
						} else {
							match_conditions.push(match);
						}
					} else if (stage.$lookup || stage.$unwind || stage.$addFields) {
						// Hoist these stages to the parent pipeline
						// For $unwind when the parent is an OR group, use preserveNullAndEmptyArrays
						// so we don't lose records that don't have the association
						if (stage.$unwind && is_or_group) {
							// Convert simple $unwind string to object form with preserveNullAndEmptyArrays
							let path = typeof stage.$unwind === 'string' 
								? stage.$unwind 
								: stage.$unwind.path;
							stage = {
				...
				// Use the extracted match conditions
				if (match_conditions.length === 1) {
					compiled_group = match_conditions;
				} else if (match_conditions.length > 1) {
					// Multiple match conditions from nested group - wrap in $and
					compiled_group = [{ $and: match_conditions }];
				} else {
					compiled_group = [];
				}
			} else {
			...
			if (compiled_group && compiled_group.length) {
	...
	// Handle associations requested via populate()
	let associations_to_select = criteria.getAssociationsToSelect();

	if (associations_to_select) {

		for (let alias in associations_to_select) {

			// Skip if already added via a WHERE clause on the association
			if (aggregate?.lookups[alias]) {
				continue;
			}

			// Get the association info
			assoc = criteria.model.getAssociation(alias);

			// Skip if association doesn't exist (and warn about it)
			if (!assoc) {
				alchemy.distinctProblem('unknown-populate-association', 'Unknown association "' + alias + '" requested in populate() for model ' + criteria.model.name);
				continue;
			}

			// Only handle BelongsTo and HasMany via $lookup for now
			// Other types (HasAndBelongsToMany, etc.) fall back to old behavior
			if (assoc.type != 'BelongsTo' && assoc.type != 'HasMany') {
				continue;
			}

			// Skip $lookup optimization when recursive > 1 for self-referencing associations
			// because $lookup doesn't handle nested association loading
			let recursive_level = criteria.options.recursive;
			if (Number.isSafeInteger(recursive_level) && recursive_level > 1) {
				// Check if this is a self-referencing association
				let assoc_model_name = alchemy.getModel(assoc.modelName, false)?.name;
				if (assoc_model_name === criteria.model.name) {
					// Skip $lookup for self-referencing with recursive > 1
					// Let the normal association loading handle it
					continue;
				}
			}
			...
			// Note: Pipeline syntax requires MongoDB 3.6+
			let lookup_stage = {
				$lookup: {
					from         : assoc_model.table,
					localField   : assoc.options.localKey,
			...
			if (assoc_select && assoc_select.fields && assoc_select.fields.length > 0) {
				...
				// Only add pipeline when we have projections (requires MongoDB 3.6+)
				lookup_stage.$lookup.pipeline = [{ $project: projection }];
			}

			aggregate.pipeline.push(lookup_stage);
	...
	// Handle associations referenced in sort
	let sort_options = criteria.options?.sort;

	if (sort_options && sort_options.length) {
		for (let sort_entry of sort_options) {
			let sort_assoc_name = sort_entry[2];
			...
			if (aggregate?.lookups?.[sort_assoc_name]) {
				continue;
			}

			// Get the association info
			assoc = criteria.model.getAssociation(sort_assoc_name);

			if (!assoc) {
				alchemy.distinctProblem('unknown-sort-association', 'Unknown association "' + sort_assoc_name + '" referenced in sort() for model ' + criteria.model.name);
				continue;
			}

			// Only handle BelongsTo and HasMany for now
			if (assoc.type != 'BelongsTo' && assoc.type != 'HasMany') {
				continue;
			}

			// Create the aggregate if needed
			getAggregate();

			aggregate.lookups[sort_assoc_name] = true;
			assoc_model = alchemy.getModel(assoc.modelName);

			// For sort, we use preserveNullAndEmptyArrays to keep records without the association
			addLookupForAssociation(aggregate, assoc, assoc_model, {
				preserveNull: true,
				addNullCheck: false
			});
	...
	// Determine the operator to use based on group type
	let group_operator = is_or_group ? '$or' : '$and';
...
function shouldStringify(entry, item) {
	...
	try {
		...
		// Field lookup can fail for computed/virtual fields or invalid paths
		// Default to not stringifying in these cases
		alchemy.distinctProblem('nosql-shouldStringify-' + entry.target_path, 'Failed to check field type for stringify: ' + entry.target_path);
		return false;
	}
}
...
NoSQL.setStatic(function convertAQLToConditions(query) {
	...
	let conditions = convertAQLTokens(tokens, (name, operator, value, not) => {
		...
		switch (value.type) {
			...
			case 'string':
				value = NoSQL.parseQuotedWildcard(value.value);
				break;
 ...
 */
NoSQL.setStatic(function parseQuotedWildcard(input) {

	input = String.parseQuoted(input);

	if (input.includes('*')) {
		input = RegExp.interpretWildcard(input, 'i');
	}
...
function convertAQLTokens(expressions, expression_converter) {
	...
	const closeGroup = () => {
		...
		// Fold the closed group into its parent right away: the `child`
		// link can only hold ONE group, so a second parenthesized sibling
		// (like `(a = 1) and (b = 2)`) used to overwrite - and silently
		// drop - the first one
		if (current_group) {
			current_group.expressions.push(simplifyGroups(group));

			if (current_group.child === group) {
				current_group.child = null;
			}
		}
	}
