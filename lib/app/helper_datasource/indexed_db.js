	...
	if (!cursor) {
		...
		if (options.sort) {
			...
			if (Array.isArray(options.sort)) {
				// Criteria sorts are arrays of [path, direction] pairs.
				// Iterating them as a {path: direction} map fed sortByPath
				// garbage: the first key sorted DESCENDING by default and
				// the real directions applied to array-index "paths"
				for (let entry of options.sort) {
					config.sort.push(entry[1]);
					config.sort.push(entry[0]);
				}
			} else {
		...
		if (options && options.wrap_result) {
			// The plain callback signature loses `available` (pledge
			// resolvers only take (err, result)), so datasource reads opt
			// in to a wrapped result instead
			return config.callback(null, {rows: result, available});
		}
