...
Mongo.setMethod(function connect() {
	...
	if (this[CONNECTION_ERROR]) {
		...
		if (error_age < retry_delay) {
			// A rejection, NOT a throw: connect() gets evaluated eagerly as
			// a waterfall argument (collection(), _read), so a sync throw
			// bypassed error handlers built for rejections - a Fallback
			// datasource stopped serving cached reads during the retry
			// window because the whole read rejected instead of falling back
			return Swift.reject(this[CONNECTION_ERROR]);
		}
	...
	return Swift.waterfall(
		...
		async collection => {
			...
			if (compiled.pipeline) {
				...
				// Apply the field selection: the plain find path passes the
				// projection as a cursor option, but this pipeline path used
				// to silently return FULL documents whenever an association
				// forced an aggregate
				if (options.projection && options.projection.length) {

					let projection = {_id: 1};

					for (let field of options.projection) {
						projection[field] = 1;
					}

					// The looked-up association data has to stay
					for (let alias in compiled.lookups) {
						projection[alias] = 1;
					}

					compiled.pipeline.push({$project: projection});
				}
...
Mongo.setMethod(function _ensureIndex(model, index, callback) {
	...
	let pledge = Swift.waterfall(
		...
		async (collection) => {
			...
			// A partial filter expression and `sparse` are mutually exclusive
			// in MongoDB. Prefer the partial filter: it indexes exactly the
			// documents matching the filter, so it does not fall for the
			// present-but-null trap that makes a sparse unique index collide
			// once two documents share a null value for the indexed field.
			if (index.options.partialFilterExpression) {
				options.partialFilterExpression = index.options.partialFilterExpression;
			} else {
				options.sparse = index.options.sparse ? true : false;
			}
