...
SystemTask.constitute(function addTaskFields() {
	...
	// Only system schedules carry a checksum, so scope the uniqueness to
	// them with a partial filter. A plain sparse index would break here:
	// a non-system task saved with a null checksum is still indexed by a
	// sparse index (sparse only skips ABSENT fields), so two such tasks
	// would collide on null.
	this.addIndex('forced_schedule_checksum', {
		unique : true,
		partialFilterExpression : {
			schedule_type : {$in: ['system_forced', 'system_fallback']},
		},
	});
});
...
SystemTask.setMethod(function beforeSave(doc) {
	...
	// The checksum only belongs to system schedules; never let a user task
	// carry one (it would be an orphan colliding with the system record
	// that legitimately owns that checksum).
	if (doc.schedule_type != 'system_forced' && doc.schedule_type != 'system_fallback') {
		doc.forced_schedule_checksum = null;
	}
