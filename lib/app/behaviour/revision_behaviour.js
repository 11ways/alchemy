...
Revision.setMethod(function beforeSave(record, options, creating) {
	...
	if (!creating) {
		...
		Model.get(that.name).find('first', {conditions: {_id: record._id}}, function(err, result) {
		...
		Model.get(that.name).find('first', {conditions: {_id: record._id}}, function(err, result) {
			...
			if (result.length) {
				...
				if (newData) {
					...
					revdata[that.behaviours.revision.RevisionModel.name] = {
						record_id: options.__data_revision_before._id,
						delta: delta
					};

					// Save the data
					that.behaviours.revision.RevisionModel.save(revdata, {allowFields: true});
				}
			}
