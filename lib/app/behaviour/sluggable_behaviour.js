...
var Sluggable = Function.inherits('Behaviour', function SluggableBehaviour(model, options) {
	...
	if (!this.targetField) {
		this.targetField = model.blueprint.addField(this.options.target, 'String');
	}
...
Sluggable.setMethod(function beforeSave(data, options, creating) {
	...
	// Leave a slug alone when not creating, unless it is set empty
	if (!creating) {
	...
	this.createSlug(data, function createdSlug(err, result) {
...
Sluggable.setMethod(function createSlug(record, callback) {
	...
	if (source && typeof source == 'object') {
		...
		if (!record[that.targetField.name]) {
			return that.generateSlug(source, callback);
		}
	}
