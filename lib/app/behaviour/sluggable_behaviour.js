...
Sluggable.setMethod(function beforeSave(data, options, creating) {
	...
	if (new_value === undefined) {
		has_new_value = 'new_value' in data;
	} else {
		has_new_value = true;
	}
...
Sluggable.setMethod(function createSlug(record, callback) {
	...
	if (source && typeof source == 'object') {
		...
		Object.each(source, function eachSource(title, key) {
			...
			if (!record[that.target_field.name] || !record[that.target_field.name][key]) {
				tasks[key] = function generateSlug(next) {
					that.generateSlug(title, key, record._id, next);
				};
			}
		});
		...
		if (!record[that.target_field.name]) {
			return that.generateSlug(record[that.source_field.name], false, record._id, callback);
		}
	}
 ...
 * @param    {String}   title
 * @param    {String}   path
 * @param    {ObjectId} for_record_id
 * @param    {Function} callback
 */
Sluggable.setMethod(function generateSlug(title, key, for_record_id, callback) {
	...
	if (typeof key == 'object') {
		callback = for_record_id;
		for_record_id = key;
		key = false;
	}

	if (typeof for_record_id == 'function') {
		callback = for_record_id;
		for_record_id = null;
	}
