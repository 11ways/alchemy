 ...
 * @version   1.0.0
 */
alchemy.castObjectId = function castObjectId(str) {
	if (str.isObjectId()) {
		return str;
	}
};
...
/**
 * Is the given input an object id?
 *
 * @author    Jelle De Loecker   <jelle@develry.be>
 * @since     1.0.4
 * @version   1.0.4
 */
alchemy.isObjectId = function isObjectId(input) {
	return !!this.castObjectId(input);
};
 ...
 * @return   {Linkup}
 */
alchemy.linkup = function linkup(type, data, cb) {
	return alchemy.server.linkup(type, data, cb);
};
...
if (hawkejs.scene.exposed.enable_websockets) {
	...
	alchemy.on('css_reload', function onReload() {
		alchemy.reloadStylesheets();
	});
}
 ...
 * @version   1.0.0
 */
alchemy.reloadStylesheets = function reloadStylesheets() {
