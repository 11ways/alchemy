...
SchemaField.setMethod(function _toDatasourceFromValueWithSubSchema(value, holder, record, sub_schema, datasource, callback) {
	...
	if (sub_schema) {
		return Pledge.Swift.done(datasource.toDatasource(sub_schema, value), callback);
	}
		...
		if (model) {
			...
			model.find('first', {document: false, fields: [this.options.schema]}, function gotRecord(err, result) {
				...
				if (!result.length) {
					...
					return Pledge.Swift.done(datasource.toDatasource(null, value), callback);
				}
				...
				Pledge.Swift.done(sub_schema, (err, sub_schema) => {
					...
					Pledge.Swift.done(datasource.toDatasource(sub_schema, value), callback);
				});
			});
