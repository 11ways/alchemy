 ...
 * Deprecated alias
 * 
 * @deprecated   Use getSubSchema (capital S) instead
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    0.2.0
 * @version  1.4.0
...
SchemaField.setMethod(function getSubschema(context) {
	alchemy.distinctProblem('deprecated-getSubschema', 'field.getSubschema() is deprecated, use field.getSubSchema() (capital S)');
...
/**
 * Convert this schema field to a JSON Schema representation.
 * Delegates to the nested schema's toJsonSchema() if available.
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    1.4.0
 * @version  1.4.0
 ...
 */
SchemaField.setMethod(function toJsonSchema() {

	// Try to get the sub-schema (without context, so only static schemas work)
	let sub_schema = this.field_schema;

	// If we have a static schema, delegate to it
	if (sub_schema && typeof sub_schema.toJsonSchema === 'function') {
		let schema = sub_schema.toJsonSchema();

		// Add title and description from this field if the schema doesn't have them
		if (this.title && !schema.title) {
			schema.title = this.title;
		}

		if (this.description && !schema.description) {
			schema.description = this.description;
		}

		// Handle arrays
		if (this.is_array || this.force_array_contents) {
			schema = {
				type: 'array',
				items: schema,
			};
	...
	// If the schema is dynamic (string path), we can't resolve it statically
