...
var SchemaField = Function.inherits('Alchemy.Field', function Schema(schema, name, options) {
	...
	if (!this.fieldSchema.parent || schema.root_schema == this.fieldSchema.parent) {
		...
		if (this.fieldSchema.parent != schema) {
			throw new Error('Unable to re-use a schema for field "' + name + '"');
		}
	}

	if (!this.fieldSchema.model_name) {
		this.fieldSchema.setModel(schema.model_name);
	}
});
...
SchemaField.setMethod(function _toApp(query, options, value, callback) {
	...
	if (recursive && this.fieldSchema && !Object.isEmpty(this.fieldSchema.associations)) {
		...
		sub_criteria.setOption('associations', this.fieldSchema.associations);
