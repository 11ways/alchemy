...
DateField.setMethod(function castCondition(value, expression) {
	...
	// A query on a specific unit sub-field (like `field.year`) must keep its
	// raw value. But our OWN 'timestamp' marker doesn't opt out: the second
	// bound of a chained range (`.gte(a).lte(b)`) used to skip conversion,
	// compiling to {$gte: <number>, $lte: <Date>} - which matches nothing
	if (expression && expression.db_property && expression.db_property != 'timestamp') {
		return value;
	}
 ...
 */
DateField.setMethod(function augmentJsonSchema(schema) {
	schema.format = 'date';
});