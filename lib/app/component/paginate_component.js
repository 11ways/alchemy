...
Component.extend(function PaginateComponent (){
	...
	this.find = function find(model, options, callback) {
		...
		if (!render) {
			throw new Error('No render object has been found!');
		}
	...
	this.processFilter = function processFilter(model, options, callback) {
			...
			for (filterField in filterParam) {
				...
				if (foreign) {
					...
					if (String(entry.value).isObjectId()) {
						query[filterField] = alchemy.castObjectId(entry.value);
					} else {

						(function(foreign, field, entry, filterField, fieldInfo) {
							tasks[tasks.length] = function foreignFinder(next) {

								var assocModel = model.getModel(foreign.modelName),
								    condition = {};

								condition[assocModel.displayField] = String(entry.value).diacriticRegex(true);

								assocModel.find('all', {debug: true, conditions: condition}, function(err, results) {
					...
					switch (field.type.toLowerCase()) {
						...
						case 'string':
							query[filterField] = String(entry.value).diacriticRegex(true);
							break;
		...
		Function.series(tasks, function() {
			callback();
		});
	};
});
