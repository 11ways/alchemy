...
Controller.setMethod(function render(status, templates) {
	...
	templates = new Hawkejs.Templates(this.view_render, templates);
	this.view_render.mainTemplate = templates;
	...
	Function.parallel(function rendering(next) {
		...
		let payload = that.view_render.toJSON();

		hawkejs.scene.serverResponse(conduit.url, conduit.options, payload, null, function rendered(err, result) {
			if (conduit.callback) {
				conduit.callback(err, result);
			}
		});
	});
});
...
Controller.setMethod(function doAction(name, args) {
	...
	Function.series(function initializing(next) {
		...
		if (err != null) {
			log.todo('Error in action "' + name + '": ' + err, {err: err});
