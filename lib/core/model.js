...
this._la = {zever:1};
	 ...
	 * Does nothing if an index on the field was already created.
	 *
	 * @author   Jelle De Loecker   <jelle@kipdola.be>
	 * @since    0.0.1
	 * @version  0.0.1
	 *
	 * @param    {boolean}  dropIndex   Drop the index? (Needed for changes)
	 */
	this.ensureIndex = function ensureIndex (dropIndex) {
		
		if (typeof dropIndex == 'undefined') dropIndex = false;
		...
		this._whenConnected({
			eval: fncString,
			args: [thisModel._indexes, thisModel.modelName.tableize()]
		}, function (err, items) {
			pr('Ensured indexes');
			pr(err);
		});
