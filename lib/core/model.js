...
var Model = global.Model = alchemy.classes.BaseClass.extend(function Model (){
	 ...
	 * @returns  {object}
	 */
	this._compileConditions = function _compileConditions (conditions) {
		...
		for (var i in conditions) {
			...
			if (fields.model && fields.model != this.modelName) {
				storeInternal = false;
			} else {
				storeInternal = true;
			}
			...
			if (typeof entry != 'string' && typeof entry != 'number' && !(entry instanceof RegExp) ) {
				...
				result = this._compileConditions(entry);
		...
		return {internal: _conditions, external: external};
	}
