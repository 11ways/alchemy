...
var Model = global.Model = alchemy.classes.BaseClass.extend(function Model (){
	...
	// Indexes settings are stored here, from blueprint
	this._index = {};
	...
	this.__extended__ = function __extended__ (parentClassName, parentClass) {
		...
		this.prototype.modelName = this.modelName;
		...
		this._compileAssociation('hasOne', this.hasOne, {foreignKey: this.modelName.foreign_key()}, {fnc: 'findOne'});
		this._compileAssociation('belongsTo', this.belongsTo, null, {fnc: 'findOne'});
		
		this.prototype.associations = this.associations;
	}
	...
	this._compileAssociation = function _compileAssociation (name, associations, defaults, overrides) {
		...
		for (i in associations) {
			...
			this.associations[modelName] = {type: name, foreignKey: foreignKey, modelName: modelName, alias: i, fnc: 'find'};
		...
		for (var fieldname in blueprintClone) {
			...
			if (typeof e == 'string') {
				...
				if (e.index) {
					...
					// Store these settings in the _index object
					this._index[fieldname] = e.index;
				}
	...
	this.ensureIndex = function ensureIndex () {
		
		var thisModel = this;
		var indexes = [];
		
		for (var indexField in this._index) {
			
			var keys = {};
			var options = this._index[indexField];
			
			keys[indexField] = 1;
			
			if (index.order == 'desc') keys[indexField] = -1;
			
			indexes.push({keys: keys, options: options});
		}
		
		this._whenConnected({
			eval: 'function (indexes){for(var i = 0; i < indexes.length; i++){}}',
			args: [indexes]
		}, function (err, items) {
			pr('Ensured indexes');
			pr(err);
	...
	this._compileConditions = function _compileConditions (query, options, findScope, level) {
		...
		var util = require('util');
		pr(util.inspect(wrap, false, null));
	...
	/**
	 * Save data of this model only to the database
	 *
	 * @author   Jelle De Loecker   <jelle@kipdola.be>
	 * @since    0.0.1
	 * @version  0.0.1
	...
	this.save = function save (data, options, callback) {
		...
		// fieldList is true by default
		if (typeof options.fieldList == 'undefined') options.fieldList = true;
		
		// Go over every record we should save
		for (var recordNr in data) {
		
			var record = data[recordNr];
		
			if (typeof data[this.modelName] != 'undefined') data = data[this.modelName];
			
			this.saveOne(record, options);
	...
	this.saveOne = function saveOne (data, options, callback) {
		...
		// This is the function we'll use to actually save the record
		var saveRecord = function saveRecord (dataToSave, extra_callback) {
			
			// @todo: implement updating records based on other unique fields
			var record = new thisModel._model(dataToSave, strictFields);
			
			record.save(function saveRecordToMongo (err, item) {
				
				extra_callback(err, item);
		...
		if (options.callbacks) {
			...
			this.beforeSave(function afterBeforeSave (record, over_options) {
				...
				if (record === false) {
					...
					saveRecord(data, function beforeAfterSave (err, item) {
...
var _get = function get (modelName) {
	...
	log.verbose('Getting model instance: ' + modelName.underline, {level: 1});
