...
var Model = global.Model = alchemy.classes.BaseClass.extend(function Model (){
	...
	this._createQuery = function _createQuery (options) {
		...
		var compiled = this._compileConditions({$and: options.conditions});
		var util = require('util');
		
		pr(util.inspect(compiled, false, null));
		
		for (var extModel in compiled.options.extModels) {
			pr('Compiling conditions for extModel: ' + extModel);
			var c = this._compileConditions({$and: options.conditions}, {modelName: extModel});
			pr(util.inspect(c, false, null));
		}
		
		
		
		options._conditions = options.internal;
		options._external = options.external;
	...
	this._compileConditions = function _compileConditions (query, options, findScope, level) {
		...
		var a = this._compileLevel(query, options, findScope, level);
		
		var util = require('util');
		
		pr('New compile conditions:');
		pr(util.inspect(a, false, null));
		
	}
	
	this._compileLevel = function _compileLevel (query, options, findScope, level) {
		
		var info = {models: [], findScope: findScope, level: level};
		var wrap = [];
		...
		for (var field in query) {
			...
			if (!(value instanceof RegExp) && (value instanceof Array || value instanceof Object)) {
				pr('Is an object:')
				pr(value);
				
				newVal = this._compileLevel(value, options, findScope, level+1);
				
				for (var i in newVal[0].models) {
					counter[newVal[0].models[i]] = true;
				}
				
			} else {
				newVal = {model: fieldsModel, field: fieldName, value: value};
			}
