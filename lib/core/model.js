...
var Model = global.Model = alchemy.classes.BaseClass.extend(function Model (){
	...
	this.save = function save (data, options, callback) {
		...
		for (recordNr in data) {
			...
			record = data[recordNr];
		
			if (typeof data[this.modelName] != 'undefined') data = data[this.modelName];
			
			// The function that will be run by async
			var saveFnc = function saveFunction (async_callback) {
				thisModel.saveOne(record, options, function saveFunctionCallback(err, item) {
					async_callback(null, {err: err, item: item});
				});
			}
		...
		async.parallel(par, function saveFinished (async_err, results) {
			...
			if (callback) {
				for (var i in results) if (results[i].err) err = true;
