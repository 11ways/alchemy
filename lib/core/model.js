...
var Model = global.Model = alchemy.classes.BaseClass.extend(function Model (){
	...
	this.save = function save (data, options, callback) {
		...
		var thisModel = this, // A scope-save link to this context
		    par = [],         // All save functions to be executed by async
		...
		for (recordNr in data) {
		
			// A recordset containing all the records for different models we need to save
			recordset = data[recordNr];
			...
			// The function that will be run by async
			(function saveOneClosure (recordset) {
				saveFnc = function saveFunction (async_callback) {
					...
					thisModel.saveOne(recordset[thisModel.modelName], options, function saveFunctionCallback(err, item) {
						...
						async_callback(null, {err: err, item: item, associated: recordset});
					});
				}
			})(recordset);
		...
		// Execute all functions besides each other, and return the results
		async.parallel(par, function saveFinished (async_err, results) {
			
			var err = null, par = [], record, assoc;
			...
			for (var i in results) {
				...
				for (var modelName in results[i].associated) {
					...
					assoc = thisModel.associations[modelName];
					record = results[i].associated[modelName];
					...
					var extraQ = (function (_model, record) {
						return function extraQSaveFunction (qcb) {
							pr('Saving extra record');
							_model.save(record, function extraQCallback (err, results) {
								qcb(err, results);
							});
			...
			if (!par.length) if (callback) {callback(err, results);}
			else {
