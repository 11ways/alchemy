...
Alchemy.setMethod(function syncDataWithServer() {
	...
	models.forEach(function eachModel(model) {
		...
		tasks.push(async function doSave(next) {
			...
			// Function.parallel ignores the returned promise of an async
			// task: a rejection would leave `next` uncalled forever, so the
			// whole sync pledge never settled and the error only surfaced
			// as an unhandled rejection. Catch and deliver it instead -
			// per-record failures just leave that record queued for the
			// next sync attempt.
			try {
				...
				for (i = 0; i < records.length; i++) {
					try {
						...
						// Still unreachable/rejected: leave it flagged and
