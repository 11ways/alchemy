 ...
 */
server.doFind = function doFind(self, table, conditions) {
	
	var cursor,
	    options = self.options;

	cursor = db[table].find(conditions);

	// Apply certain options if this is the main model
	if (self.main == table) {

		// If a valid sort object is given, apply it to the cursor
		// @todo: allow sorting associated data?
		if (typeof self.options.sort == 'object') {
			cursor.sort(self.options.sort);
		}
	}

	// @todo: improve this by using cursors later on
	// now we do it this way to get the speed up
	if (isEmpty(conditions) && table == self.main) {
...
server.doCondition = function doCondition(self, assoc, compiled) {
	...
	if (!self.results[assoc.alias]) {
		self.results[assoc.alias] = doFind(self, assoc.table, compiled);
	} else {
...
server.ensureMainData = function ensureMainData(self) {
	...
	if (!results[self.mainAlias]) {
		...
		results[self.mainAlias] = doFind(self, self.main, condition);
	}
