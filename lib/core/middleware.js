...
Alchemy.setMethod(function findAssetPath(assetFile, directories, callback) {
	...
	// Helper function to handle fs.stat errors appropriately
	function handleStatError(err, filepath) {
		if (!err) return;
		
		// ENOENT (file not found) and ENOTDIR are expected - file just doesn't exist here
		...
		alchemy.distinctProblem(
			'fs-stat-error-' + err.code,
			'Unexpected fs.stat error while looking for asset: ' + assetFile,
			{error: err, path: filepath, repeat_after: 60000, register_error: true}
		);
	}
	...
	Function.while(function test() {
		...
		Function.series(function checkOriginal(nextc) {
			...
			fs.stat(path, function gotFileStats(err, stats) {
				...
				// Log unexpected errors (not ENOENT/ENOTDIR)
				handleStatError(err, path);
...
Alchemy.setMethod(function getCompiledLessPath(lessPath, options, callback) {
	...
	fs.readFile(lessPath, {encoding: 'utf8'}, function gotLessCode(err, source) {
		...
		less.render(source, render_options, function gotCss(err, cssTree) {
			...
			if (err != null) {
				log.error('Error compiling LESS:', err);
				return callback(err);
			}
...
async function createAlchemyScss() {
	...
	for (let css_name of required_stylesheets) {
		...
		let path = result?.source?.relative_path;
...
const logStylesheetMessage = (prefix, message, options) => {

	if (!alchemy.settings.debugging.debug) {
		return;
	}

	if (!alchemy.settings.frontend?.stylesheet?.allow_warnings) {
		return;
	}

	console.warn(prefix, message, options);
};

const logSassWarning = (message, options) => {
	logStylesheetMessage('SASS Warning:', message, options);
};

const logSassDebug = (message, options) => {
	logStylesheetMessage('SASS Debug:', message, options);
};
