	...
	Object.each(collections, function(collection, alias) {
		...
		tasks[tasks.length] = function(next) {
			...
			switch (config.type) {
				...
				case 'BelongsTo':
					assocKey = '_id';
					localKey = config.options.foreignKey;
					break;
			...
			collection.model.datasource.read(collection.model, conds, {fields: fields}, function(err, items) {
				...
				if (that.debug) {
					console.log('Querying assoc model', collection.model.name, conds, 'result:', err, items, 'adding key', assocKey);
				}
...
DbQuery.setMethod(function execute(callback) {
	...
	this.normalize(function gotNormalizedConditions(err, conditions) {
		...
		that.model.datasource.read(that.model, conditions, options, function(err, items, available) {
