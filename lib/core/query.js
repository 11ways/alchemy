 ...
 */
global.DbQuery = Function.inherits('Alchemy.Base', function DbQuery(model, options, level) {
...
DbQuery.setMethod(function normalize(callback) {
	...
	// This fixes a bug where certain $or conditions
	// get grouped when they shouldn't be
	if (Array.isArray(this.options.conditions)) {
		divided = this.options.conditions;
	} else {
