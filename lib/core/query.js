var util      = alchemy.use('util'),
    nedbm     = alchemy.use('nedb/lib/model'),
 ...
 */
GLOBAL.DbQuery = Function.inherits('Informer', function DbQuery(model, options, level) {
 ...
 * @version  0.1.0
 */
DbQuery.setMethod(function processContain(callback) {
...
DbQuery.setMethod(function normalize(callback) {
	...
	this.normalizeGroup('$and', normalized, this.options.conditions, function doneRootGroup(err) {
...
DbQuery.setMethod(function normalizeGroup(type, target, conditions, callback) {
	...
	if (Array.isArray(conditions)) {
		...
		Function.parallel(tasks, function(err, arr) {
	...
	Object.each(conditions, function(value, fieldPath) {
		...
		if (fieldConfig.config.castForQuery) {
			value = fieldConfig.config.castForQuery(value);
		}
	...
	Object.each(collections, function eachCollection(collection, alias) {
		...
		tasks[tasks.length] = function getExternalCollection(next) {
			...
			var config = that.model.blueprint.associations[alias],
			    fields = {},
			    conds  = {},
			    assocKey,
			    localKey,
			...
			collection.model.datasource.read(collection.model, conds, {fields: fields}, function(err, items) {
...
DbQuery.setMethod(function execute(callback) {
	...
	this.normalize(function gotNormalizedConditions(err, conditions) {
		...
		if (!that.model.datasource) {
			return callback(new Error('Could not perform query: No valid datasource is configured for ' + that.model.name));
		}

		that.model.emit('reading_datasource', that);

		// Query the main model
		that.model.datasource.read(that.model, conditions, options, function gotItems(err, items, available) {
				...
				if (that.options.locale && that.options.locale !== true) {
					...
					for (j = 0; j < that.translation_conditions.length; j++) {
						...
						for (prefix in translations) {
							...
							if (nedbm.match(temp, condition.query)) {
								found_prefix = prefix;
								break FindPrefix;
							}
						}
...
DbQuery.setMethod(function addField(fieldPath, obj) {
	...
	var associations = this.model.schema.associations,
	    fieldConfig,
	    assoc;
 ...
 * @return   {Object}
 */
DbQuery.setMethod(function getFieldInfo(path, reportError) {
	...
	if (modelName == first || modelName.replace(/Datum$/, 'Data') == first) {
		...
		// Try to get the alias model by this name
		if (first[0] == first[0].toUpperCase() && first.match(/[A-Z]/)) {
			aliasModel = model.getAliasModel(first);
		}

		// The aliasModel was found!
		if (aliasModel) {
			alias = first;
			pieces.shift();
		} else {
	...
	if (!aliasModel) {
		aliasModel = model.getAliasModel(alias);
	}

	table = aliasModel.table;

	config = model.getField(pieces[0]);
