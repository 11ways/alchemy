...
function submit(packet) {
	...
	packet.http_port = alchemy.settings.config.port;
 ...
 * @version  1.0.0
 */
mevents.on('discover_services', function gotDiscoverRequest(requirements, packet, callback) {

	// If the required services are not met, do nothing
	if (Object.keys(services).shared(requirements).length != requirements.length) {
		return;
	}
