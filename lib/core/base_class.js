...
var BaseClass = function BaseClass() {
	...
	this.testparent = function testparent() {
		pr('Baseclass testparent!', true);
	}
	...
	this.parent = function __parent__(functionname, useCallingArguments) {
		...
		if (!functionname) functionname = p.name;
		console.log(this.constructor.name)
		console.log(p)
		// Where to find the parent function or property
		_parent = alchemy.classes[p.__parentClass__];
		
		if (_parent) possibleTarget = _parent[functionname];
...
BaseClass.extend = function extend(_extension, _options, _three, _callback) {
	...
	// We apply it later on inside new_constructor
	...
	for (var i in new_constructor.prototype) {
		
		if (typeof new_constructor.prototype[i] == 'function') {
