...
var BaseClass = function BaseClass() {
	 ...
	 */
	this.__parent = function __parent__(parentClassName, functionname, useCallingArguments) {
		
		// Baseclass doesn't have any useful parent functions
		if (parentClassName == 'BaseClass') return;

		//// Where to find the parent function or property
		var _parent = alchemy.classes[parentClassName];
		...
		if (typeof possibleTarget == 'function') {
			...
			if (useCallingArguments && (useCallingArguments instanceof Array || typeof useCallingArguments.length != 'undefined')) {
			...
			log.warn('Could not find parent property ' + functionname.bold + ' from ' + this.name.bold + ' looking in ' + parentClassName.bold + ' (Request context: ' + this.name + ')', {level: 1});
		}
	};
}
...
BaseClass.extend = function extend(_extension, _options, _three, _callback) {
	...
	var internal_constructor = function internal_constructor () {
		...
		var arg = arguments[0];
		pr('Instancing ' + this.name + ' (origin: ' + origin.name + ')');
		
		this.parent = new_constructor.parent;
		...
		if (typeof arg == 'object' && arg && arg.__extending__ === true) {
		...
		if (typeof arg == 'object' && arg && arg.__instancing__ === true) {
		...
		if (typeof arg == 'object' && arg && (arg.__extending__ === true || arg.__instancing__ === true)) {
	...
	new_constructor.parent = function() {
		var args = Array.prototype.slice.call(arguments);
		log.warn('Calling parent function ' + arguments[0] + ' in class ' + origin.name);
		args.unshift(origin.name);
		this.__parent.apply(this, args);
	}
	...
	//new_constructor.prototype.parent = new_constructor.parent = (function(__parentClass__) {
	//	return function() {
	//		console.log('--');
	//		var args = Array.prototype.slice.call(arguments);
	//		args.unshift(__parentClass__);
	//		this.__parent.apply(this, args);
	//	}
	//}(origin.name));
