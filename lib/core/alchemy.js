...
global.Alchemy = Function.inherits('Alchemy.Base', function Alchemy() {
	...
	// Hohenheim (Java port) spawns us rather than fork()s us, so Node's
	// built-in IPC channel above is unreachable. Instead it exports the
	// loopback TCP port of its IpcChannel via HOHENHEIM_IPC_PORT and speaks
	// newline-delimited flat JSON. Bridge that stream into the same emit()
	// surface so `janeway_propose_geometry` and similar work unchanged.
	let hohenheimIpcPort = parseInt(process.env.HOHENHEIM_IPC_PORT, 10);
	if (hohenheimIpcPort > 0) {
		try {
			let net = require('net');
			let socket = net.connect(hohenheimIpcPort, '127.0.0.1');
			let buffer = '';
			...
			that.hohenheim_ipc_socket = socket;
		} catch (err) {
			// Bridge is best-effort; non-Hohenheim children must still boot.
		}
	}
...
Alchemy.setMethod(function setSetting(path, value) {
	...
	// Once `alchemy.settings` has become a static snapshot (the settings
	// stage - or any System.Setting DB apply - replaces the live proxy
	// with a `toObject()` copy) every write must refresh it: gating on
	// `started` alone left setSetting calls between the snapshot and the
	// server stage invisible in `alchemy.settings`.
	if (this.started || this._settings_is_snapshot) {
...
Alchemy.setMethod(function refreshSettingsObject() {
	this._settings_is_snapshot = true;
...
Alchemy.setMethod(function loadSettings() {
	...
	// Handle AI devmode flag (CLI only - not a setting for security reasons)
	if (this.argv['ai-devmode']) {
		// Store directly on alchemy instance, not in settings
		this.ai_devmode_enabled = true;
		// Disable devwatch (AI controls restarts)
		this.setSetting('debugging.kill_on_file_change', false);
	}
...
Alchemy.setMethod(function findModule(moduleName, options) {
	...
	if (result.err) {
		try {
			...
			log.error('Error searching for module:', moduleName, err);
			return;
		}
	}
