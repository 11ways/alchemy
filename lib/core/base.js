 ...
 * Broken from 1.2.4 to 1.4.8: the truthy `indexOf` check made every
 * SERVER class take the "already client-side" early return (indexOf
 * returns -1, which is truthy), and namespaces carry no leading dot so
 * the '.Alchemy.' replacement never matched either - it always handed
 * back the server class.
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    1.2.4
 * @version  1.4.8
...
Base.setStatic(function getClientClass() {
	...
	if (this.namespace == 'Alchemy.Client' || this.namespace.indexOf('Alchemy.Client.') === 0) {
		return this;
	}

	if (this.namespace != 'Alchemy' && this.namespace.indexOf('Alchemy.') !== 0) {
		return;
	}

	// Add the Client part of the namespace (no leading dot: a namespace
	// is 'Alchemy' or 'Alchemy.Document', never '.Alchemy.…')
	let namespace = 'Alchemy.Client' + this.namespace.slice('Alchemy'.length);
