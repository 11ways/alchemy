 ...
 *
 * @return   {boolean}  If we were able to load in the directory
 */
Alchemy.setMethod(function usePath(dirPath, options) {
	...
	if (options.modular && options._level == 0) {
		...
		if (options.views !== false) {
			alchemy.addViewDirectory(libpath.resolve(dirPath, 'view'), options.weight);
		}

		// Main asset directory
		alchemy.addAssetDirectory(libpath.resolve(dirPath, 'assets'), options.weight);

		// Asset scripts
		if (options.scripts !== false) {
			alchemy.addScriptDirectory(libpath.resolve(dirPath, 'assets', 'scripts'), options.weight);
		}

		// Asset stylesheets
		if (options.less !== false) {
			alchemy.addStylesheetDirectory(libpath.resolve(dirPath, 'assets', 'stylesheets'), options.weight);

			// Also add the public folder, so less files in there can also be compiled
			alchemy.addStylesheetDirectory(libpath.resolve(dirPath, 'public'), options.weight);
		}

		// Fonts
		if (options.fonts !== false) {
			alchemy.addFontDirectory(libpath.resolve(dirPath, 'assets', 'fonts'), options.weight);
		}

		// Images
		if (options.images !== false) {
			alchemy.addImageDirectory(libpath.resolve(dirPath, 'assets', 'images'), options.weight);
		}

		// public folders (going to /public/)
		if (options.public !== false) {
			alchemy.addPublicDirectory(libpath.resolve(dirPath, 'public'), options.weight);
		}

		// Root folders (/)
		if (options.root !== false) {
			alchemy.addRootDirectory(libpath.resolve(dirPath, 'root'), options.weight);
		}
...
Alchemy.setMethod(function _usePath(dir_path, options) {
	...
	for (i = 0; i < files.length; i++) {
		...
		file_path = libpath.resolve(dir_path, file_name);
		...
		let extension = libpath.extname(file_path);
...
prive.loadRegexFile = function loadRegexFile (dirPath, pattern, multiple) {
	...
	for (fileCount in files) {
		...
		filePath = libpath.resolve(dirPath, fileName);
...
prive.loadHelpers = function loadHelpers(...path_pieces) {
	...
	try {
		...
		for (fileCount in helperFiles) {
			...
			try {
				filePath = libpath.resolve(dir_path, name);
 ...
 *
 * @return   {boolean}  False if no match was found, true if any matched
 */
