...
Alchemy.setMethod(function useAppPath(dir_path, options) {
	...
	eachDirectoryEntryStats(dir_path, (file_path, name, stat, extension) =>  {
...
Alchemy.setMethod(function useHelperPath(dir_path, options) {
	...
	eachDirectoryEntryStats(dir_path, (file_path, name, stat, extension) =>  {
		...
		if (!isValidJsFileExtension(extension)) {
			return;
		}
...
function eachDirectoryEntryStats(path, callback) {
	...
	for (let file of files) {
		...
		callback(file_path, file, stat, extension);
	}
}

/**
 * Is this a valid JS file extension?
 * 
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    1.4.0
 * @version  1.4.0
 */
const isValidJsFileExtension = (extension) => {
