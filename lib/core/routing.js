...
var dispatchController = function dispatchController (req, res, c) {
	...
	alchemy.doMiddlewareAfter(req, res, function() {
		...
		// Create the implanted controller
		implantedController = alchemy.implant(c.controller, {render: renderCallback});
		
		// The view hasn't been rendered yet
		renderCallback.rendered = false;
		renderCallback.rendering = false;

		// The locale as requested by the path
		renderCallback.locale = that.locale;

		// Locale fallback, should be deduced from the request headers later
		renderCallback.localeUser = ['nl', 'en', 'fr'];

		// If no locale was requested, get the first localeUser
		if (!that.locale) renderCallback.locale = renderCallback.localeUser.shift();
		...
		renderCallback.redirect = function redirect (_status, _url) {
			...
			implantedController._launchComponents('beforeRedirect', renderCallback, function afterComponentBeforeRedirect (_status, _url) {
				...
				implantedController.beforeRedirect(function beforeRedirectNext (_status, _url) {
		...
		implantedController._launchComponents('initialize', renderCallback, function afterComponentInitialize () {
			
			// First call the beforeAction callback
			implantedController.beforeAction(function beforeActionNext (viewVars) {
				
				if (typeof viewVars === 'object') alchemy.inject(finalViewVars, viewVars);
				
				// Now call the component startup methods
				implantedController._launchComponents('startup', renderCallback, function afterComponentStartup () {

					// Call the actual action
					implantedController[c.actionName](renderCallback);
