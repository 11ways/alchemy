...
alchemy.addRoute = function addRoute(path, callback, method, order) {
	...
	alchemy.sputnik.after('routes', function addRouteToExpress() {
...
alchemy.connect = function connect(name, paths, options) {
	...
	// Store the first path under the name
	linkmap[name] = Array.cast(paths)[0];
...
Prefix.determine = function determinePrefix(alchemyRoute, clientLanguages, session) {
	...
	// If a force prefix is set, use that
	if (Prefix.forcePrefix && prefixes[Prefix.forcePrefix]) {
		return prefixes[Prefix.forcePrefix];
	}
...
var dispatchController = function dispatchController(req, res, _c) {
	...
	// See if there's a "hajax" get parameter
	hid = url.indexOf('hajax=');

	// If there is, remove it
	if (hid > -1) {
		url = url.slice(0, hid-1);
	}
	...
	alchemy.doMiddlewareAfter(req, res, function() {
		...
		renderCallback.redirect = function redirect (_status, _url) {
			...
			augmentedController._launchComponents('beforeRedirect', renderCallback, function afterComponentBeforeRedirect (_status, _url) {
				...
				augmentedController.beforeRedirect(function beforeRedirectNext (_status, _url) {
					...
					// Annoying IE ajax cache fix
					if (renderCallback.get && req.headers['user-agent'].indexOf('MSIE') > -1 || req.headers['user-agent'].indexOf('Trident') > -1) {

						if (cont_url.indexOf('?') == -1) {
							cont_url += '?';
						} else {
							cont_url += '&';
						}
		...
		renderCallback.serveFile = function serveFile(filepath, filename, nocache) {
			...
			async.series(task, function(err, taskresult) {
				...
				c.controller._launchComponents('shutdown', renderCallback, function afterComponentBeforeRender() {
					...
					c.controller.afterRender(function afterRenderNext (newHtml) {
						...
						if (typeof html !== 'string') {
							...
							html = alchemy.hawkejs.stringify(html);
