...
var dispatchController = function dispatchController(req, res, c) {
	...
	alchemy.doMiddlewareAfter(req, res, function() {
		...
		renderCallback.serveFile = function serveFile(filepath) {

			var task = {};

			// Get some information on this file and cache it
			if (typeof servecache[filepath] === 'undefined') {
			...
			async.series(task, function() {
				...
				var result = servecache[filepath];
				...
				if (result.err) {
					...
					res.writeHead(200, {
						...
						'Content-Length': result.info.size
					});
