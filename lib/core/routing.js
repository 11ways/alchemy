...
alchemy.connect = function connect (name, paths, options) {
	...
	for (locale in paths) {
		...
		alchemy.addRoute(fullPath, routeDispatcher.bind(context), options.method);
	}
}
...
var routeDispatcher = function routeDispatcher (req, res) {
	var R = Route.get(this.options.routeClass);
...
var dispatchController = function dispatchController (req, res, c) {
	...
	renderCallback = function(viewVars) {return secondComponentStage(req, res, c, finalViewVars, viewVars, renderCallback);}
...
pr('Calling the action')
pr(String(c.controller[c.actionName]));
