...
Debugger.setMethod(function mark(stop, message) {
	...
	// only stop that mark
	if (stop === false && message) {
		for (i = 0; i < this.children.length; i++) {
			temp = this.children[i];

			if (temp.data.title == message) {
				return temp.stop();
			}
		}
 ...
 */
alchemy.pickTranslation = function pickTranslation(_prefix, choices, allow_empty) {
 ...
 */
alchemy.getMimetype = function getMimetype(filePath, callback) {
 ...
 */
alchemy.request = function request(url, options, callback) {
 ...
 * @return   {Boolean}
 */
alchemy.isStream = function isStream(obj) {
