...
alchemy.findModule = function findModule(moduleName) {
	...
	if (result.err) {
		...
		if (!moduledirs) {
			moduledirs = fs.readdirSync(path.resolve(PATH_ROOT, 'node_modules'));
		}

		// Go over every directory in the main node_modules folder
		for (i = 0; moduledirs.length; i++) {
			...
			try {
				...
				modulePath = require.resolve(path.resolve(PATH_ROOT, 'node_modules', key, 'node_modules', moduleName));
...
alchemy.ObjectId = require('mongoose').Types.ObjectId;
...
alchemy.castObjectId = function castObjectId(obj) {
	...
	if (type === 'object' && obj.constructor && obj.constructor.name === 'ObjectID') {
		return obj;
