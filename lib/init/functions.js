...
alchemy.useApp = function useApp (pathToDirs) {
	...
	var pathArguments, path = alchemy.use('path');
...
/**
 * Prepare a plugin for use, but do not load it in just yet
 *
 * @author   Jelle De Loecker   <jelle@kipdola.be>
 * @since    0.0.1
 * @version  0.0.1
...
alchemy.usePlugin = function usePlugin (name, options) {
	...
	var pathToPlugin, pluginStat, fs = alchemy.use('fs');

	pathToPlugin = alchemy.pathResolve(PATH_ROOT, plugins, name);
