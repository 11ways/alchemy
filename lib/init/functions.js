...
Informer.setMethod(function debug(type, data, async) {
	...
	console.log('DEBUG', type, async, data);

	if (!this.conduit) {
		console.log(this.constructor.name, 'does not have conduit attached');
		return;
	} else {
		console.log('\n\n\nCONDUIT FOUND\n\n\n');
	}

	item = new Debugger(type, async);
	item.data = data;

	console.log('Pushing to debuglog', item, this.conduit)
	...
	if (async) {
		return function() {
			item.end = Blast.performanceNow();
		};
	}
});

var Debugger = Function.inherits(function Debugger(type, async) {
...
Debugger.setMethod(function mark(message) {
	this.labels.push({message: message, time: Blast.performanceNow()});
});

Debugger.setMethod(function debug(type, data, async) {

	var item = new Debugger(type, async);
	item.data = data;
...
Debugger.setMethod(function stop() {
	this.end = Blast.performanceNow();
});
