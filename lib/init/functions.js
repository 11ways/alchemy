...
/**
 * Implant certain properties into an instance's context,
 ...
 * @return   {Object}
 */
alchemy.implant = function implant(original, addition) {

	var OriginalContextWrapper, implantedContext;
	...
	var implantedContext = new OriginalContextWrapper();

	// If a (valid) addition is given, implant it
	if (typeof addition === 'object') {
		// Now inject the additions into the new context,
		// this will leave the original context untouched
		alchemy.inject(implantedContext, addition);

		// Also add the additions one level deeper,
		// that way we can retrieve what was implanted
		alchemy.inject(implantedContext, {__implantation__: addition});

		// If this instance has an implanted() method, run it
		if (typeof implantedContext.implanted === 'function') {
			implantedContext.implanted();
		}
	}
	
	// Finally return the implantedContext
	return implantedContext;
};
