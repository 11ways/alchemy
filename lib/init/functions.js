...
/**
 * Merge 2 objects together, a recursive inject
 *
 * @author   Jelle De Loecker   <jelle@kipdola.be>
 * @since    0.0.1
 * @version  0.0.1
		...
		for (key in extension) {
			...
			if (typeof extension[key] === 'object' && extension[key] !== null) {
				...
				if (Array.isArray(item)) {
					// If the target is already an array, merge them
					if (Array.isArray(target[key])) {
						target[key].concat(item);
					} else {
 ...
 * @version  0.0.1
 */
alchemy.objectPath = function objectPath(obj, path, value) {
