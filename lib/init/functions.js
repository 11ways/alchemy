...
Alchemy.setMethod(function exposeDefaultStaticVariables() {
	...
	if (!alchemy.settings.websockets || alchemy.settings.websockets == 'optional') {
	...
	if (alchemy.environment == 'dev') {
		hawkejs.exposeStatic('app_version', ''+Date.now());
	} else {
		hawkejs.exposeStatic('app_version', alchemy.package.version);
	}
});
