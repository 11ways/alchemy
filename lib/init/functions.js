...
var runQueue = {before: {}, after: {}};

/**
 * Run something before a certain runlevel
 *
 * @param   {String}   name       The name of the runlevel
 * @param   {Integer}  order      The order of the function to execute
 * @param   {Function} fnc        The function to execute
 * @param   {Function} callback   The callback function to execute when done
 */
alchemy.before = function before (name, order, fnc, callback) {
	alchemy._onRunlevel('before', name, order, fnc, callback);
}

/**
 * Run something after a certain runlevel
 *
 * @param   {String}   name       The name of the runlevel
 * @param   {Integer}  order      The order of the function to execute
 * @param   {Function} fnc        The function to execute
 * @param   {Function} callback   The callback function to execute when done
 */
alchemy.after = function after (name, order, fnc, callback) {
	alchemy._onRunlevel('after', name, order, fnc, callback);
}

/**
 * Run something before or after a certain runlevel
 *
 ...
 * @param   {Function} callback   The callback function to execute when done
 */
alchemy._onRunlevel = function (when, name, order, fnc, callback) {
	
	var rQ = runQueue[when];
	
	if (typeof alchemy.runlevels[name] == 'undefined') {
		rQ[name] = {};
		alchemy.runlevels[name] = false;
	}
	
	if (typeof rQ[name][order] == 'undefined') rQ[name][order] = [];
	
	if (alchemy.runlevels[name]) {
		if (fnc) fnc(callback);
	} else {
		if (fnc) rQ[name][order].push({function: fnc, callback: callback});
	}
