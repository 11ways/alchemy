...
if (process.stdout.isTTY) {
	...
	echo = function echo(msg) {
		console.log.apply(null, Array.cast(arguments));
	};
} else {
	echo = function echo(msg) {
...
(function() {
	...
	levels = {
		...
		todo: {output: 'todo', color: 'red'}
	};

	_logLabels.levels = levels;
...
var log = global.log = function log(options, message, obj) {
	...
	if (Janeway.started) {
		temp = [_logLabels[name].bold, trace];
		Janeway.debug(temp)
		temp = temp.concat(args);

		if (options.extra) {
			temp.push(callerInfo.text);
		}

		echo.apply(null, temp);
...
log.error = function error(message, options) {
	...
	if (options.err) {
		...
		info = getCallerInfo(0, options.err);
