...
var log = global.log = function log (level, message, obj) {
	...
	process.stdout.write(_logLabels[level].bold);
	...
	for (var i = 0; i < args.length; i++) {
		...
		if (typeof args[i] != 'string') args[i] = util.inspect(args[i], false, null);
...
log.stack = function log_stack() {

	var stack = getCallerInfo(5);
...
log.error = function error (message, options) {
	...
	if (!options.force && alchemy.settings.config.logLevel < 0 && alchemy.settings.config.logLevel != null) return false;
	
	if (typeof options.stack == 'undefined') options.stack = false;
	
	var trace = '';
	
	if (alchemy.settings.config.logTrace || alchemy.settings.config.logTraceError === true || options.stack === true) {
...
log.warn = function warn (message, options) {
	...
	if (alchemy.settings.config.logLevel < 1 && alchemy.settings.config.logLevel != null) return;
	...
	if (alchemy.settings.config.logTrace || alchemy.settings.config.logTraceWarn === true || options.stack === true) {
		if (typeof options.level == 'undefined') options.level = 0;
		var callerInfo = getCallerInfo(options.level);
		trace = '['.grey + callerInfo.file.bold + ':' + callerInfo.line.bold + ']'.grey;
		trace += ' ';
...
log.info = function info (message, options) {
	...
	if (alchemy.settings.config.logLevel < 2 && alchemy.settings.config.logLevel != null) return;
	...
	if (alchemy.settings.config.logTrace || alchemy.settings.config.logTraceInfo === true || options.stack === true) {
		if (typeof options.level == 'undefined') options.level = 0;
		var callerInfo = getCallerInfo(options.level);
		trace = '['.grey + callerInfo.file.bold + ':' + callerInfo.line.bold + ']'.grey;
		trace += ' ';
...
log.debug = function debug (message, options) {
	...
	if (alchemy.settings.config.logLevel < 3 && alchemy.settings.config.logLevel != null) return;
	...
	if (alchemy.settings.config.logTrace || alchemy.settings.config.logTraceDebug === true || options.stack === true) {
		if (typeof options.level == 'undefined') options.level = 0;
		var callerInfo = getCallerInfo(options.level);
		trace = '['.grey + callerInfo.file.bold + ':' + callerInfo.line.bold + ']'.grey;
		trace += ' ';
...
log.verbose = function verbose (message, options) {
	...
	if (alchemy.settings.config.logLevel < 4 && alchemy.settings.config.logLevel != null) return;
	...
	if (alchemy.settings.config.logTrace || alchemy.settings.config.logTraceVerbose === true || options.stack === true) {
		if (typeof options.level == 'undefined') options.level = 0;
		var callerInfo = getCallerInfo(options.level);
		trace = '['.grey + callerInfo.file.bold + ':' + callerInfo.line.bold + ']'.grey;
		trace += ' ';
...
global.pr = function pr(message, attn) {
	if (attn) {
		return log.attn(message, {level: 1});
	} else {
