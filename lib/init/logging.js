...
/**
 * Set specific throttle configuration
 *
 * @author   Jelle De Loecker   <jelle@develry.be>
 * @since    0.2.2
 * @version  0.2.2
 ...
 * @param    {Object}   config
 */
log.setLessConfig = function setLessConfig(id, config) {

	if (!config) {
		config = {};
	}

	less_log_ids[id] = Function.throttle(function doLog(id) {
		...
		log.info(args, {level: 5});
	}, config.ms || 1100, true);
};

/**
 * Log something without overloading the log output
 *
 * @author   Jelle De Loecker   <jelle@develry.be>
 * @since    0.2.2
 * @version  0.2.2
 ...
 * @param    {String}   id
 */
log.less = function less(id) {

	if (!less_log_ids[id]) {
		log.setLessConfig(id);
	}

	less_log_ids[id].apply(null, arguments);
};
...
if (!Blast.isNW && process.stdout.isTTY && !process.env.DISABLE_JANEWAY && process.argv.indexOf('--disable-janeway') == -1 && alchemy.settings.janeway !== false) {

	Janeway.started = true;
	Janeway.start();

	if (alchemy.settings.title) {
		title = alchemy.settings.title;

		if (alchemy.settings.titleized) {
			title = 'Alchemy: ' + title;
		}

		Janeway.setTitle(title);
	}
