...
State.prototype.begin = function begin () {
	if (!this.begun) {
		...
		// Execute the predefined states
		if (typeof this.triage.preStates[this.name] != 'undefined') {
			for (var i in this.triage.preStates[this.name]) {
				this.triage.preStates[this.name][i]();
			}
		}
...
var Triage = function Triage (options) {
	...
	// Pre-defined state functions
	this.preStates = {};
	
}

/**
 * Do the provided function as a state.
 * This will begin & end a state.
 *
 * @author   Jelle De Loecker   <jelle@kipdola.be>
 * @since    0.0.1
 * @version  0.0.1
 ...
 * @param    {Function} fnc       The state function
 */
Triage.prototype.executeAsState = function executeAsState (name, fnc) {
	this.begin(name);
	fnc();
	this.end(name);
}
...
/**
 * Define the provided function as a state, but execute later.
 * This will begin & end a state.
 *
 * @author   Jelle De Loecker   <jelle@kipdola.be>
 * @since    0.0.1
 ...
 * @param    {Function} fnc       The state function
 */
Triage.prototype.defineState = function defineState (name, fnc) {
	
	if (typeof this.preStates[name] == 'undefined') this.preStates[name] = [];
	
	// Add this function
	this.preStates[name].push(fnc);
}
...
Triage.prototype.begin = function begin (name) {
	...
	// And begin the state (which fires the 'before' functions and predefined functions)
	state.begin();
 ...
 */
Triage.prototype.wait = function wait (stateName) {
	...
	if (!state.open) {
		...
		return state.caller.bind(state);
	}
	
	return function(){log.warn('Returnd empty wait callback')};
}
