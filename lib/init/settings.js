...
if (!settings.name) {
	settings.name = alchemy.package.name;
}

if (settings.title == null) {
	if (alchemy.package.title) {
		// Allow users to set the title in their package file
		settings.title = alchemy.package.title;
...
if (port > 49151) {
	...
	if (port > 65535) {
		die(portError + 'there is no port higher than 65535. Please use ports below 49151.');
	} else {
		die(portError + 'it\'s an ephemeral port. Please use ports below 49151.');
	}
}
