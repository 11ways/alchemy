...
alchemy.usePath = function usePath(dirPath, options) {
	...
	if (options.modular && options._level == 0) {
		...
		modularRegex.push(/^view$|^helper$|^element$|^plugins$|^assets$/);
 ...
 */
alchemy.addAssetDirectory = function addAssetDirectory(dirPath, weight) {
 ...
 * @param   {Number}  weight     The weighted importance [10]
 */
alchemy.addImageDirectory = function addImageDirectory(dirPath, weight) {
...
alchemy.usePlugin = function usePlugin(name, options) {
	...
	possiblePaths.push(alchemy.pathResolve(PATH_APP, 'plugins', name));

	// Look for the plugin in every possible path
	foundPath = possiblePaths.some(function eachPath(value) {

		try {
			pathToPlugin = value;
			pluginStat = fs.lstatSync(pathToPlugin);
			isDir = pluginStat.isDirectory() || pluginStat.isSymbolicLink();
