...
alchemy.usePath = function usePath(dirPath, options) {
	...
	if (options.modular && options._level == 0) {
		...
		if (options.scripts !== false) {
			prive.addScriptDirectory(path.resolve(dirPath, 'assets', 'scripts'));
		}
		...
		if (options.public !== false) {
			prive.addPublicDirectory(path.resolve(dirPath, 'public'), options.weight);
		}
 ...
 */
prive.addScriptDirectory = function addScriptDirectory(dirPath, weight) {
 ...
 * @param   {Number}  weight     The weighted importance [10]
 */
prive.addPublicDirectory = function addPublicDirectory(dirPath, weight) {
