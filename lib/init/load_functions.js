...
alchemy.usePath = function usePath(dirPath, options) {
	...
	if (options.modular && options._level == 0) {
		...
		if (options.views !== false) {
			prive.addViewDirectory(path.resolve(dirPath, 'view'), options.weight);
		}

		// Asset stylesheets
		if (options.less !== false) {
			prive.addStylesheetDirectory(path.resolve(dirPath, 'assets', 'stylesheets'), options.weight);
		}
		//prive.loadLess(path.resolve(dirPath, 'assets', 'stylesheets'), alchemy.sputnik.wait('stylesheets'));
 ...
 */
prive.addStylesheetDirectory = function addStylesheetDirectory(dirPath, weight) {
...
prive.loadAssetDirectory = function loadAssetDirectory(type, filepath, callback, subdir) {
	...
	fs.readdir(filepath, function (err, files){
		...
		Function.series(srun, function(err, results) {
			callback();
		});
 ...
 * @param   {Number}  weight     The weighted importance [10]
 */
prive.addViewDirectory = function addViewDirectory(dirPath, weight) {
...
prive.loadPublic = function loadPublic (dirPath, callback, order) {
	...
	fs.exists(dirPath, function pathExistsResult(exists) {
...
alchemy.usePlugin = function usePlugin(name, options) {
	...
	if (isDir) {
		...
		try {
			...
			alchemy.useOnce(alchemy.pathResolve(pathToPlugin, 'bootstrap.js'), {silent: true});
		} catch (err) {
...
alchemy.setStatic = function setStatic () {
	...
	var keys = Object.keys(staticQueue).map(Number);
	...
	for (var i in keys) {
		...
		for (var nr in q) {
			...
			if (p.publicPath === '/') {
				...
				pr('Using "' + p.localPath.replace(cwd+'/', '').bold + '" as "' + p.publicPath.bold + '"');
