...
alchemy.usePath = function usePath (dirPath, options) {
	...
	if (options.modular && options._level == 0) {
		...
		// Hawkejs views
		prive.loadViews(path.resolve(dirPath, 'view'), alchemy.sputnik.wait('views'));
		
		// LESS stylesheets
		prive.loadLess(path.resolve(dirPath, 'assets', 'less'), alchemy.sputnik.wait('stylesheets'));

		// public folders (going to /public/)
		prive.loadPublic(path.resolve(dirPath, 'public'), alchemy.sputnik.wait('static'), options.order);
...
prive.loadLess = function loadLess (filepath, callback, subdir) {
	...
	var lessPath = path.resolve(PATH_TEMP, 'less');
	...
	fs.readdir(filepath, function (err, files){
		...
		if (err) {
			log.warn('LESS directory does not exists: ' + filepath);
				...
				srun.push(function(asynccallback) {
					...
					fs.stat(path.resolve(filepath, filename), function(err, stats){
						...
						if (stats.isDirectory()) {
							...
							prive.loadLess(dirpath, function(){asynccallback(null)}, subdir)
							
						} else {

							mkdirp(lessPath + '/' + subdir + '/', function (error) {
							
								// Open the original file
								var origin = fs.createReadStream(filepath + '/' + filename);
								
								// Open the destination file
								var destination = fs.createWriteStream(lessPath + '/' + subdir + '/' + filename);     
								
								origin.on('end', function(err) {
									
									if (err) {
										log.warn('LESS file could not be loaded: ' + filename);
									}
...
alchemy.static = function _static (publicPath, localPath, order) {
	...
	pr('Wanting to set static folder ' + localPath);
...
alchemy.useOnce = function useOne (dirPath) {
	...
	if (dirPath.indexOf('.js') == -1) {
		log.error('Tried to load in non javascript file: ' + dirPath);
		return false;
	}
