...
Alchemy.setMethod(function usePath(dirPath, options) {
	...
	if (options.modular && options._level == 0) {
		...
		modularRegex.push(/^view$|^helper$|^helper_datasource$|^helper_field$|^helper_document$|^helper_model$|^helper_controller$|^helper_component$|^helper_validator$|^element$|^plugins$|^assets$/);
				...
				if (fs.lstatSync(filePath).isDirectory()) {
					...
					alchemy.hawkejs.load(filePath, {
						arguments : {
							...
							values : [ Classes.Hawkejs,   Blast,   Blast.Bound,  Classes,   Blast.Collection.Function]
						}
					});
				}
			} catch (err) {
...
Alchemy.setMethod(function usePlugin(name, options) {
	...
	if (options.path_to_plugin) {
		...
		for (key in alchemy.plugins) {
			...
			possible_paths.push(alchemy.pathResolve(temp.__path, 'node_modules'));
		}
	}
