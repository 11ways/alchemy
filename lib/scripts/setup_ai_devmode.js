...
module.exports = function setupAiDevmode() {
	...
	AiDevmode.setAction(async function inspect(conduit) {
		...
		try {
			...
			// Expression form first (the expression's value is the result);
			// on a syntax error fall back to statement form, so multiline
			// multi-statement input works too (it must `return` its own
			// result). The newlines around the expression keep a trailing
			// line comment from eating the wrapper.
			let asyncFn;
