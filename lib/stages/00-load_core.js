 ...
 * @type     {Alchemy.Stages.Stage}
 */
const preload_modules = load_core.createStage('preload_modules', async () => {
	/**
	 * Pre-load basic requirements
	 */
	requireCorePath('scripts', 'preload_modules');

	// Load mime module now & cache it
	let libmime = await alchemy.use('mime');
	alchemy.modules['mime'] = libmime.Mime;
});
