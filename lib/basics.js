...
var queryLevel = function queryLevel (condition, currentModel) {
	...
	for (i = 1; i < condition.length; i++) {
		...
		var r = db[modelConditions.model].find(query).toArray();
		...
		(function (name) {
			...
			db_connections[name].once('open', function callback () {
				...
				db_connections[name].db.eval(e, function() {
					log.info('Stored "' + 'transmuteQuery'.bold + '" and "' + 'queryLevel'.bold + '" procedures on MongoDB server ' + name.bold);
				});
