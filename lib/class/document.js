 ...
 * @version  1.1.5
 *
 * @param    {Model|String|Schema}   model_param
 */
Document.setStatic(function getDocumentClass(model_param) {
	...
	if (model_param instanceof Classes.Alchemy.Schema) {
		schema = model_param;
		model_param = schema.model_class || schema.model_name;
	}
	...
	if (schema) {
		document_name = 'SchemaField' + schema.name.camelize() + Crypto.pseudoHex();
	} else {
	...
	if (DocClass == null) {
		...
		} else if (!model.super || model.super.name == 'Model' || !model.super.Document) {
			parent_path = 'Alchemy.Document.Document';
		} else {
