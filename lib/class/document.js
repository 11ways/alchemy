...
Document.setMethod(function dryClone(wm) {
	...
	result = new this.constructor(record, JSON.clone(this.getCleanOptions(), wm));

	// Copy over attributes
	result.$_attributes = JSON.clone(this.$_attributes);
...
Document.setMethod(function remove(callback) {
	...
	return this.$model.remove(this._id, callback);
});
 ...
 * @return   {Pledge}
 */
Document.setMethod(function addAssociatedData(options, callback) {
	...
	return model.addAssociatedDataToRecord(options, {
...
Document.setMethod(function revert(revisions) {
	...
	pledge = Function.series(async function getDeltas(next) {
		...
		records = await revision_model.find('all', options);
