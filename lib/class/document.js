...
Document.setStatic(function unDry(obj, cloned) {
	...
	// (toDry stores it as `$attributes`: reading `$_attributes` here meant
	// the original_record - and with it the document's dirty state - was
	// silently dropped on every dry/undry round-trip)
	if (obj.$attributes) {
		result.$_attributes = obj.$attributes;
	}

	// Restore the internal hold data (translated fields and the like)
	if (obj.$hold) {
...
Document.setStatic(function setComputedFieldGetter(name) {
	this.setProperty(name, function getComputedFieldValue() {
		...
		log.error('Can not set computed field "' + name + '" to', value);
	});
});
