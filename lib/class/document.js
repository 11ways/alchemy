 ...
 */
var Document = Function.inherits(['Alchemy.Base', 'Iterator', 'Array'], 'Alchemy', function Document(record, options) {
...
Document.setStatic(function setFieldGetter(name, getter, setter) {
	...
	if (typeof getter != 'function') {
		getter = function getFieldValue() {
			...
			return this._main_model_data[name];
		};

		setter = function setFieldValue(value) {
			this._main_model_data[name] = value;
		};
	} else {
...
Document.setStatic(function setAliasGetter(name) {
	...
	this.setProperty(name, function getAliasObject() {
		return this._current_record_data && this._current_record_data[name];
	}, function setAliasObject(value) {
		this._current_record_data[name] = value;
	});
});
 ...
 * @version  0.5.0
 */
Document.setProperty(function _main_model_data() {
	return this._records[this._iterNextIndex][this.modelName];
});
 ...
 * @version  0.5.0
 */
Document.setProperty(function _current_record_data() {
...
/**
 * Init this document.
 * Users can add their own constructor functionality in here
 *
 * @author   Jelle De Loecker <jelle@develry.be>
 * @since    0.2.1
 * @version  0.2.1
 */
Document.setMethod(function init() {});
...
Document.setMethod(function get(alias, field) {
	...
	if (this._current_record_data && this._current_record_data[alias]) {
		return this._current_record_data[alias][field];
	}
});
...
Document.setMethod(function save(data, options, callback) {
	...
	function updateDoc(err, saveResult) {
		...
		main._id = alchemy.castObjectId(saveResult[0][that.modelName]._id);
...
Document.setMethod(function current() {
	return this._current_record_data || {};
});
 ...
 * @version  0.5.0
 */
Document.setMethod(function forEach(fnc, context) {
 ...
 */
Document.setMethod(function some(fnc, context) {
	...
	for (i = 0; i < length; i++) {
		...
		// Call the function with that object
		output = fnc.call(context, temp, i, this);
...
/**
 * Iterate over the records
 * until the callback returns a falsy value
 *
 * @author   Jelle De Loecker <jelle@develry.be>
 * @since    0.5.0
 * @version  0.5.0
 ...
 * @param    {Object}     context
 */
Document.setMethod(function every(fnc, context) {
...
/**
 * Return an array with individual documents
 *
 * @author   Jelle De Loecker <jelle@develry.be>
 * @since    0.3.3
 * @version  0.4.0
 */
Document.setMethod(function toArray() {
	...
	for (i = 0; i < this.length; i++) {
		temp = new this.constructor([this[i]], Object.assign({}, options));
		result.push(temp);
	}
...
Document.setMethod(function getDisplayFieldValue(options) {
	...
	display_field = Array.cast(this._model.displayField);
 ...
 * @param    {Function}   callback
 */
Document.setMethod(function findNextBatch(callback) {

	var that = this,
	    conditions,
	    options;

	if (!this._options.query_options) {
		return callback(new Error('No earlier query options were saved'));
	}

	options = Object.assign({}, this._options.query_options);
	options.query = null;
	...
	this._options.model.find('all', options, callback);
});