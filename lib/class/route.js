...
const Route = Function.inherits('Alchemy.Base', function Route(router, paths, options) {
	...
	// The optional custom conduit class to use for this route
	this.conduit_class = null;
...
/**
 * Resolve and cache the custom conduit class if specified in options
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    1.4.0
 * @version  1.4.0
 */
Route.setMethod(function resolveConduitClass() {

	if (!this.options.conduit_class) {
		return;
	}

	this.conduit_class = Object.path(Classes, this.options.conduit_class);

	if (!this.conduit_class) {
		throw new Error('Unknown conduit class "' + this.options.conduit_class + '" - must be a valid class path like "Alchemy.Conduit.Http"');
	}
});
...
Route.setMethod(async function callHandler(conduit, parameters) {
	...
	// Transform conduit if route specifies a custom class
	if (this.conduit_class && conduit.constructor !== this.conduit_class) {
		conduit = new this.conduit_class(conduit);
	}
