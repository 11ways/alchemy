...
Conduit.setMethod(function parseRoute(after_route) {
	...
	if (!this.route) {
		...
		if (temp) {
			...
			if (this.method == 'head') {
				let get_route = this.router.getRouteBySectionPath('get', this.section, this.sectionPath, this.prefix, after_route);
