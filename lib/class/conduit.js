...
Conduit.setDeprecatedProperty('viewRender',      'view_render');
 ...
 * @version  1.0.6
 */
Conduit.prepareProperty(function view_render() {
	var view_render = alchemy.hawkejs.createViewRender();

	// Pass version info for styles & scripts caching
	view_render.internal('version_info', alchemy.package.version);
	view_render.expose('version_info', alchemy.package.version);
...
Conduit.setMethod(function prepareViewRender() {
	...
	this.view_render.server_var('conduit', this);

	// Let the ViewRender get some request info
	this.view_render.prepare(this.request, this.response);

	// Pass url parameters to the client
	this.view_render.internal('urlparams', this.params);
	this.view_render.internal('url', this.url);

	if (this.route) {
		this.view_render.internal('route', this.route.name);
	}
});
...
Conduit.setMethod(function error(status, message, printError) {
	...
	if (this.view_render && (this.ajax || (this.headers.accept && this.headers.accept.indexOf('html') > -1))) {
		this.status = status;
...
Conduit.setMethod(function render(templateName, options, callback) {
	...
	if (options.clientRender != null) {
		this.view_render.clientRender = options.clientRender;
	}

	this.view_render.beginRender(templates, function afterRender(err, html) {
...
Conduit.setMethod(function registerBindings(arr) {
	...
	if (arr) {
		...
		data_ids = this.view_render.live_bindings;
	}
...
Conduit.setMethod(function setTheme(name) {
	this.theme = name;
	this.view_render.setTheme(name);
});
