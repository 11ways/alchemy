...
Conduit.setMethod(function loopback(args, callback) {
	...
	// @TODO: clean this up in the loopback class itself
	// (Might even have to do it for non-href loopbacks)
	if (args.href) {
		if (args.href instanceof RURL) {
			result.original_url = args.href;
		} else {
			result.original_url = RURL.parse(args.href);
		}

		result.url = result.original_url;
		result.original_path = result.url.path;
		result.original_pathname = result.url.pathname;
		result.route = null;

		result.parseShortcuts();
		result.parseLanguages();
		result.parsePrefix();
		result.parseSection();

		promise = result.parseRoute();
	}
