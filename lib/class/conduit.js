...
Conduit.setMethod(function _postpone(options) {
	...
	let postponement = session.postpone(this);

	// Set the location header where the client should look at later
	response.setHeader('Location', postponement.url);
	response.setHeader('Content-Type', 'text/html');

	if (options.expected_duration) {
		response.setHeader('Expected-Duration', Number(options.expected_duration / 1000).toFixed(2));
	}
	...
	if (status !== false) {
		...
		response.writeHead(status || 202);
	}

	let end_message = options.end_message;

	// End the response if wanted
	if (end_message !== false) {
		if (!end_message) {
			end_message = 'The response has been postponed, you can find it at <a href="' + postponement.url + '">' + postponement.url + '</a>';
		}
 ...
 * @return   {Boolean}
 */
Conduit.setMethod(function shouldBeDelayed() {
	...
	if (!this.route.can_be_postponed) {
		return false;
	}
