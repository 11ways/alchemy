...
var Conduit = Informer.extend(function Conduit(req, res) {
	...
	this._debugObject = this.debug('Initialize Conduit');
...
Conduit.setMethod(function callMiddleware() {
	...
	var that = this,
	    ...
	    middleDebug = this.debug('middleware', {title: 'Doing middleware'}),
	    routeDebug;
...
Conduit.setMethod(function end(message) {
	...
	if (alchemy.settings.config.cache !== false && (cache == null || cache != 'no-cache')) {
...
Conduit.setMethod(function serveFile(path, options) {
	...
	if (!stats.path) {
		return this.notFound(new Error('No file to serve'));
	}
	...
	Function.parallel(tasks, function gotFileInfo() {
		...
		if (stats.err) {
			return that.notFound(stats.err);
		}

		if (!stats.path) {
			return that.notFound(new Error('File not found'));
		}

		// Check the if-modified-since header if it's supplied
		if (alchemy.settings.config.cache !== false && that.headers['if-modified-since'] != null) {
		...
		if (isStream) {
			path.pipe(that.response);
			return;
		}

		// Stream the file
		 fs.createReadStream(path, {bufferSize: 64*1024}).pipe(that.response);
	});
});
