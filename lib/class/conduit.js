 ...
 * @param    {Router}            router
 */
var Conduit = Function.inherits('Informer', function Conduit(req, res, router) {
 ...
 * @version  0.2.0
 */
Conduit.setMethod(function loopback(name, body, callback) {

	var result = new alchemy.classes.LoopbackConduit(this);
	...
	console.log('Calling route', name, result);
...
Conduit.setMethod(function error(status, message, printError) {
	...
	if (printError === false) {
		...
		log.error(subject + ':\n' + message, {err: new Error('Unknown error')});
	}
