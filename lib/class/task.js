...
Task.setProperty(function has_stopped() {
	// A REQUESTED stop counts too: this property is what executors poll
	// (`if (this.has_stopped) return;`), and nothing else ever exposed
	// `need_stop` to them - so stop() was unobservable for any task
	// without a custom doStop()
	return this[STATUS] == STOPPED || this.need_stop === true;
});
...
Task.setMethod(async function start(payload) {
	...
	try {
		...
		// Persist the executor's outcome: without it, a run that did
		// nothing is indistinguishable from one that did everything.
		if (result !== undefined && !document.result) {
			try {
				document.result = JSON.clone(result);
			} catch (clone_err) {
				// A non-cloneable result must never break the history save
				document.result = '(result could not be serialized: ' + clone_err.message + ')';
			}
		}
...
Task.setMethod(async function stop() {
	...
	// A paused executor is parked on the pause pledge, which only resume()
	// settles - so stopping a paused task was a permanent deadlock: the
	// executor could never observe `need_stop`, the start promise never
	// settled, and the task's cron schedule stopped repeating for the
	// process lifetime. Release it so it can see the stop request.
	if (this.is_paused) {
