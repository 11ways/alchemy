 ...
 */
RouterClass.setMethod(function getRouteBySectionPath(conduit, method, section, path, prefix, last_match) {
	...
	for (i = 0; i < routes.length; i++) {
		...
		temp = route.match(conduit, method, path, prefix);
		...
		if (temp) {
			...
			if (temp.then) {
				...
				temp.then(function gotResult(value) {
					if (value) {
						pledge.resolve({
							route               : route,
							definition          : value.definition,
							parameters          : value.parameters,
							original_parameters : value.original_parameters,
							parameters_array    : value.parameters_array
						});
					} else {
						pledge.resolve(null);
					}
				...
				return {
					...
					original_parameters : temp.original_parameters,
					parameters_array    : temp.parameters_array
				};
			}
		}
	}
