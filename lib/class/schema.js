	...
	return Function.series(function waitForModelClass(next) {
		...
		STAGES.afterStages('datasource', function afterDs() {
			...
			if (that.model_class) {
				...
				if (!datasource) {
					log.error('Failed to get', that.model_class.prototype.dbConfig, 'datasource');
...
Schema.setMethod(function addAssociation(type, relation_config, alias, modelName, options) {
	...
	if (is_internal) {
		...
		if (!this.getField(options.localKey)) {
			...
			this.addAssociationFieldToSchema(options.localKey, type, field_options, doc_class, client_doc);
		}
	...
	this.addAssociationFieldToSchema(alias, 'AssociationAlias', null, doc_class, client_doc);
