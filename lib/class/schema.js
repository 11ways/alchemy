...
Schema.setMethod(function setModel(model) {
	...
	model_name = constructor.modelName || constructor.name.beforeLast('Model') || constructor.name;
...
Schema.setMethod(function addRule(main_field, rule_name, options) {
	...
	if (rule_name) {
		...
		default_options = ValidationRules[rule_name];
...
Schema.setMethod(function addIndex(_field, _options) {
	...
	if (this.modelClass) {
		datasource = Datasource.get(this.modelClass.prototype.dbConfig);

		if (!datasource) {
			throw new Error('Could not get datasource "' + this.modelClass.prototype.dbConfig + '"');
		}

		datasource.ensureIndex(this.modelClass, this.indexes[options.name], function ensuredIndex(err, result) {
			if (err) {
...
Schema.setMethod(function addAssociation(type, alias, modelName, options) {
	...
	if (constructor) {
		doc_class = Classes.Alchemy.Document.getDocumentClass(constructor);
	}
