...
Schema.setMethod(function getDatasource() {
	...
	return Function.parallel(function waitForModelClass(next) {
		...
		alchemy.sputnik.after('datasources', function afterDs() {
			...
			if (!datasource) {
				console.log('Failed to get', that.modelClass.prototype.dbConfig, 'datasource?');
				return next(new Error('Could not get datasource "' + that.modelClass.prototype.dbConfig + '"'));
			}
