...
Schema.setMethod(function setModel(model) {
	...
	modelName = modelName || modelName.beforeLast('Model');
...
Schema.setMethod(function addAssociation(type, alias, modelName, options) {
	...
	if (locality == 'internal') {
		...
		if (alchemy.classes[this.modelName]) {
			alchemy.classes.Document.getDocumentClass(alchemy.classes[this.modelName]).setFieldGetter(options.localKey);
		}
	}
	...
	if (alchemy.classes[this.modelName]) {
		alchemy.classes.Document.getDocumentClass(alchemy.classes[this.modelName]).setAliasGetter(alias);
	}
