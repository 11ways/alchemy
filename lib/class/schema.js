...
Schema.setMethod(function addField(name, type, options) {
	...
	field = Classes.Alchemy.Field.Field.getMember(type);
...
Schema.setMethod(function addIndex(_field, _options) {
	...
	this.getDatasource().done(function gotDs(err, datasource) {
...
Schema.setMethod(function getDatasource() {
	...
	this.afterOnce('has_model_class', function hasModelClass() {
		...
		var datasource = Datasource.get(that.modelClass.prototype.dbConfig);

		if (!datasource) {
			return pledge.reject(new Error('Could not get datasource "' + that.modelClass.prototype.dbConfig + '"'));
		}

		pledge.resolve(datasource);
	});
...
Schema.setMethod(function addAssociation(type, alias, modelName, options) {
	...
	if (locality == 'internal' && !this.getField(options.localKey)) {
