...
var Model = global.Model = alchemy.classes.BaseClass.extend(function Model (){
	...
	/**
	 * Run this before this.init(),
	 * sets up several properties
	 */
	...
	/**
	 * What to do when the model is being extended
	 *
	 * @author   Jelle De Loecker   <jelle@kipdola.be>
	 * @since    0.0.1
	 * @version  0.0.1
...
pr('Extended!', true)
	...
	this.fieldType = function fieldType(fieldName) {
		...
		if (this.blueprint[fieldName] && this.blueprint[fieldName].type) {
			return this.blueprint[fieldName].type.toLowerCase();
		}
	...
	this._createSchema = function _createSchema(blueprint) {
		...
		schema = new mongoose.Schema(blueprintClone, {collection: this.useTable});
			...
			thisModel.beforeFind(function beforeFindNext (status) {
				...
				thisModel._whenConnected({
					...
					if (err) {
						...
						// before limit & offset
						payload.available = retval.info.available;
					}					
	...
	this._fireAfterFind = function _fireAfterFind (payload, err, tableItems, findCallback) {
		...
		for (modelName in foundModels) {
			...
			// Mongodb stores the results under alias names,
			// so retrieve their model names first
			if (modelName in this.aliases) {
				modelName = this.aliases[modelName];
			}
			...
			if ((field == '$or' || field == '$and')
				...
				if (this.blueprint[fieldName].type == mongoose.Schema.Types.ObjectId ||
					...
					if (typeof value === 'string') {
						value = mongoose.mongo.BSONPure.ObjectID(value);
					}
				}
	...
	this.afterFind = function afterFind(next, err, results, primary) {
		this._launchBehaviours('afterFind', next, err, results, primary);
	};
	...
	this.saveErrorHandler = function saveErrorHandler(err, result, input, callback) {
		...
		var obj, fieldName, errors, blueprint, validationType;
		...
		if (err && this.render) {
			...
			if (result.length && result[0]['err']) {
				...
				for (fieldName in err) {

					validationType = err[fieldName].type;
	...
	this._launchBehaviourMethod = function _launchBehaviourMethod(behaviourName, methodName, callback, args) {
		...
		if (this._behaviours[behaviourName][methodName]) {
			...
			var clonedArgs = args.splice(0);

			// Push the callback on top
			clonedArgs.unshift(callback);
			
			this._behaviours[behaviourName][methodName].apply(this, clonedArgs);
		} else {
			callback();
		}
	};
 ...
 * @version  0.0.1
 */
BaseClass.prototype.getModel = function getModel(modelName, autoCreate) {
...
Model.extend = function extend (name, options, fnc) {
	...
	if (this.name === 'Model') {
		if (options.base || typeof alchemy.classes.AppModel == 'undefined') {
			...
			return alchemy.classes.AppModel._extend(name, options, fnc);
		}
	} else {
