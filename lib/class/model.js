 ...
 */
Model.setProperty('is_abstract_class', true);

/**
 * This wrapper class starts a new group
 */
Model.setProperty('starts_new_group', true);
...
Model.setMethod(function createDocument(data) {
	...
	var doc = new this.constructor.Document(data, {model: this, singular: true});
	...
	pledge = Function.series(function doBeforeType(next) {
		...
		that.emit('found', options, queryItems, function afterFound(err) {
			...
			if (options.document !== false) {
				queryItems = new that.constructor.Document(queryItems, {model: that, query_options: options});
