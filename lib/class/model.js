...
var Model = global.Model = alchemy.create(function Model() {
	...
	this.saveOne = function saveOne(data, options, callback) {
		...
		this.prepareRecord(data, function(err, data) {
	...
	/**
	 * Save the given data as-is in the collection
	 *
	 * @author   Jelle De Loecker   <jelle@codedor.be>
	 * @since    0.1.0
	 * @version  0.1.0
	 ...
	 * @param    {Function}   callback
	 */
	this.prepareRecord = function prepareRecord(data, callback) {

		var that = this;

		// Look for an id first
		this.auditRecord(data, function afterAudit(err, record) {
			...
			if (!record._id) {
				...
				tasks.fetch = function(next) {
					that.queryCollection({_id: record._id}, function(err, item) {
	 ...
	 * @param    {Function}   callback
	 */
	this.auditRecord = function auditRecord(data, callback) {
		...
		if (typeof data._id === 'undefined') {
			...
			async.parallel(tasks, function(err, indexResults) {
				...
				if (!alchemy.isEmpty(indexResults)) {
