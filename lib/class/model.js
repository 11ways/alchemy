...
var Model = global.Model = alchemy.classes.BaseClass.extend(function Model (){
	...
	this._fireAfterFind = function _fireAfterFind (payload, err, tableItems, findCallback) {
		...
		for (modelName in foundModels) {
			...
			pr('>>>>>>>>>>>>>>>>>>>>'.blue.bold);
			pr('Disabled:: ' + this.disableTranslations);
			//pr(this.__augment__);

			m = alchemy.augment(m, {test: 1});

			pr('Augment: ' + this.__augment__.disableTranslations);
			pr('Disabled:: ' + m.disableTranslations);
			pr(m.__augmentLevel, true);
			
			(function(m) {
				tasks.push(function nonPrimaryAfterFind (moreItems, nonPrimaryCallback) {
					pr('Launching non-primary afterfind of ' + m.name, true);
