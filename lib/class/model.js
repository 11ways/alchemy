var expirable   = alchemy.use('expirable'),
    ...
    hash        = alchemy.use('murmurhash3js').x64.hash128,
    createdModel;
...
Model.staticCompose('schema', function createSchema(doNext) {
	...
	return schema;
}, ['addEnumValues', 'setEnumValues', 'belongsTo', 'hasOneParent', 'hasAndBelongsToMany', 'hasMany', 'hasOneChild', 'addIndex']);
...
Model.setStatic(function addField(name, type, options) {
	...
	result = this.blueprint.addField(name, type, options);
...
Model.setStatic(function addAssociation(type, alias, modelName, options) {
	var data = this.blueprint.addAssociation(type, alias, modelName, options);
 ...
 * @return   {Document}
 */
Model.setMethod(function createDocument() {

	var doc = new this.constructor.Document();
...
Model.setMethod(function getAliasModel(alias) {
	...
	if (config) {
		return this.getModel(config.modelName);
	} else {
		return this.getModel(alias);
	}
});
...
Model.setMethod(function translateItems(items, options, callback) {
	...
	// Get the prefix
	if (options.locale) {
		prefix = Array.cast(options.locale);

		if (conduit && conduit.prefix) {
			prefix = prefix.concat(Array.cast(conduit.prefix));
		}
	} else if (conduit) {
		...
		for (j = 0; j < collection.length; j++) {
			...
			for (fieldName in this.schema.translatableFields) {
				...
				if (typeof found === 'undefined' || found === '') {
					found = alchemy.pickTranslation(conduit.locales, record[fieldName], true);
				}
...
Model.setMethod(function addAssociatedDataToRecord(options, query, item, callback) {
	...
	Function.parallel(4, aliases, function(err, list) {
...
Model.get = function get(name) {
	...
	constructor = alchemy.classes[name + 'Model'] || alchemy.classes[name] || alchemy.classes[String(name).modelName() + 'Model'];
