 ...
 * @version  0.3.0
 */
var Model = Function.inherits('Alchemy.Base', function Model(options) {
...
Model.constitute(function setModelName() {
	this.model_name = this.name.beforeLast('Model');
...
Model.prepareStaticProperty('Document', function getDocumentClass() {
	return Classes.Alchemy.Document.getDocumentClass(this);
});
...
/**
 * The deprecated modelName property
 *
 * @type {String}
 */
Model.setProperty(function modelName() {
	log.warn('modelName is deprecated, use model_name');
...
Model.setStatic(function addField(name, type, options) {
	...
	if (!field.is_private) {
		this.ClientDocument.setFieldGetter(name);
	}
...
Model.setStatic(function getClientConfig() {
	...
	result = {
		...
		fields : this.schema.getClientConfig()
	};
	...
	pledge = Function.series(function doBeforeType(next) {
		...
		that.emit('found', options, queryItems, function afterFound(err) {
			...
			if (options.document !== false) {
				queryItems = that.createDocumentList(queryItems, {query_options: options});
