...
Model.setMethod(function ensureIds(list, callback) {
	...
	return Function.forEach.parallel(list, function checkEntry(entry, key, next) {
		...
		if (!id) {
			return next(new Error('`Model#ensureIds()` can\'t ensure an entry without an _id'));
		}
...
Model.setMethod(function auditRecord(data, options, callback) {
	...
	if (schema && data._id == null && options.audit !== false) {
		tasks = {};
		results = {};
...
Model.setMethod(function convertRecordToDatasourceFormat(record, options, callback) {
	...
	pledge = this.datasource.toDatasource(this, data, options);
...
Model.setMethod(function remove(id, callback) {
	...
	if (!id) {
		pledge.reject(new Error('Invalid ObjectId given!'));
		return pledge;
	}
