...
var Model = global.Model = alchemy.classes.BaseClass.extend(function Model (){
			...
			if (typeof e.type === 'string') {
				...
				switch (e.type.toLowerCase()) {
					...
					default:
						...
						if (mongoose.Schema.Types[e.type]) {
	...
	this._saveAssociatedData = function _saveAssociatedData (tasks_err, results, callback) {
		...
		for (i in results) {
			...
			for (modelName in results[i].associated) {
				
				log.verbose('Getting association for model "' + modelName.bold.underline + '"');
				
				assoc = this.associations[modelName];
				records = results[i].associated[modelName];

				if (!assoc) {
					log.error('Tried to get association "' + modelName.bold.underline + '" which does not exist in model ' + this.modelName);
