 ...
 * @type   {Schema}
 */
Model.staticCompose('blueprint', function createBlueprint(doNext) {
	...
	// This has to be scheduled next, because addField would call createBlueprint
...
Model.setProperty('name', function name() {
	return this._name || this.constructor.name.beforeLast('Model');
...
Model.setMethod(function save(_data, _options, _callback) {
	...
	Function.while(function test() {
		...
		callback(err, new that.constructor.Document(results));
	});
});
