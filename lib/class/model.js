var expirable   = alchemy.use('expirable'),
    validate    = alchemy.use('mongoose-validator').validate,
    mongoose    = alchemy.use('mongoose'),
...
var Model = global.Model = alchemy.create(function Model() {
	...
	this.init = function init(options) {
		...
		if (this.useTable) {
			...
			this._createSchema(this.blueprint);
	...
	this.withCollection = function withCollection(method) {
		...
		this.connection.collection(this.useTable, function(err, collection) {
	...
	this.queryCollection = function queryCollection(selector, options, callback) {
		...
		serialized = alchemy.stringify({selector: selector, options: options});
		...
		if (cacheResult) {
			...
			// Return the result
			callback(cacheResult.err, result);
			...
			thisModel.connection.collection(thisModel.useTable, function(err, collection) {
				...
				cursor.toArray(function(err, items) {
					...
					if (thisModel.cache) {
						thisModel.cache.set(serialized, {err: err, items: items});
					}

					callback(err, thisModel.decodeBson(items));
				});
			});
		}
	};
	...
	this.queryOrCache = function queryOrCache(queryOptions, callback) {
		...
		if (cacheResult) {
			...
			thisModel._whenConnected({
				...
				// Cache the recordset using BSON
				if (thisModel.cache) {
			...
			thisModel.beforeFind(function beforeFindNext (status) {
				...
				thisModel.queryOrCache(query.result, function (err, response) {
					...
					if (err) {
						...
						if (!response.documents[0].ok) {
							log.error('Database query failed: ' + response.documents[0].errmsg);
	...
	this.remove = function remove(id, callback) {
		...
		this.find('first', {condition: {_id: id}}, function(err, results) {

			if (!results.length) {
				return callback();
			}

			thisModel.beforeRemove(function(response) {

				if (response !== false) {
					thisModel._model.remove({_id: mongoose.Types.ObjectId(id)}, function(err) {
