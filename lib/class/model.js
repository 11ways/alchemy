		...
		if (model.prototype.add_basic_fields !== false) {
			...
			doNext(function addSchemaBasics() {
				try {
					...
					log.error('Error adding basic fields to model', model.name);
					throw err;
				}
			});
		}
...
Model.setMethod(function saveRecord(document, options, callback) {
	...
	pledge = Function.series(function doAudit(next) {
		...
		if (options.debug) {
			log.debug('Saving document', document, 'Creating?', creating);
		}
...
Model.setMethod(function auditRecord(document, options, callback) {
	...
	if (schema && document[this.primary_key] == null && options.audit !== false) {
		...
		if (options.debug) {
			log.debug('Pre-save audit record', document);
		}

		schema.eachAlternateIndex(document, function iterIndex(index, indexName) {

			if (options.debug) {
				log.debug('Checking alternate index', indexName);
			}
		...
		Function.parallel(tasks, function doneAudit(err) {
			...
			if (options.debug) {
				log.debug('Audit done, found pk:', document[that.primary_key]);
			}
...
Model.setMethod(function remove(id, callback) {
	...
	// Does any behaviour define a remove hook?
	let has_behaviour_hooks = false;

	if (this.behaviours) {
		for (let key in this.behaviours) {
			let behaviour = this.behaviours[key];

			if (typeof behaviour.beforeRemove == 'function' || typeof behaviour.afterRemove == 'function') {
				has_behaviour_hooks = true;
				break;
			}
		}
	}

	// 1.4.0 renamed the emitted event from 'removed' to 'afterRemove', so
	// listeners of either name (and behaviour hooks) have to count here
	let has_remove_events = typeof this.beforeRemove == 'function'
		|| typeof this.afterRemove == 'function'
		|| has_behaviour_hooks
		|| this.listeners('removed').length
		|| this.listeners('afterRemove').length
		|| this.listeners('beforeRemove').length
		|| this.listeners('removing').length;
	...
	if (has_remove_events) {
		...
		tasks.push(next => {
			// A data event: `callOrNext` only reached the model method,
			// silently skipping behaviour beforeRemove hooks
			this.issueDataEvent('beforeRemove', [doc], next);
		});
	}
	...
	Function.series(tasks, function done(err) {
		...
		Swift.done(that.datasource.remove(context), function afterRemove(err, result) {
			...
			if (has_remove_events) {
				that.issueDataEvent('afterRemove', [doc, result], function issued() {
					// Also emit the legacy 'removed' event for old listeners
					that.emit('removed', doc, result, () => pledge.resolve(result));
				});
			} else {
...
Model.setMethod(function eachRecord(options, task, callback) {
	...
	this.find('all', options, function gotRecords(err, result) {
		...
		Function.parallel(parallel_limit, tasks, function done(err) {
			...
			// AND-ed with (not written into) the given conditions: the old
			// in-place assignment mutated the CALLER's conditions object and
			// clobbered any caller condition on the primary key from the
			// second page on
			next_options.conditions = {
				$and: [options.conditions, {[that.primary_key]: {$gt: last_id}}],
			};
