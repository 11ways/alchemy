...
Controller.prepareProperty(function model() {
	...
	Class = Classes.Alchemy.Model.Model.getMember(this.name);

	// If the class was found, get a model instance
	if (Class) {
...
Controller.setAction(async function readDatasource(conduit) {
	...
	let that = this;
	let conditions = conduit.param('conditions');
	let options = conduit.param('options');
	let query = new Blast.Classes.Alchemy.DbQuery(model, {conditions: conditions});
