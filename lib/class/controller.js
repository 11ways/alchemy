...
var Controller = global.Controller = alchemy.classes.BaseClass.extend(function Controller (){
	...
	this.component = function getComponent(classifiedName) {
		...
		// If there is an implantation here, cache the component
		if (this[cache]) {
			implanted = this[cache];
		} else {
			implanted = alchemy.implant(this[classifiedName], this.__implantation__);
		}

		if (!this[cache] && this.__implantation__) {
			this[cache] = implanted;
		}
...
/**
 * Get an implanted controller
 *
 * @author   Jelle De Loecker   <jelle@kipdola.be>
 * @since    0.0.1
 * @version  0.0.1
 */
BaseClass.prototype.getController = function getController(controllerName) {

	// Get the model instance
	var instance = Controller.get(controllerName);
