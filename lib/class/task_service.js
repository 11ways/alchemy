...
Service.setMethod(function getTaskSchedules(constructor) {
	...
	if (typeof constructor == 'string') {
		...
		constructor = Classes.Alchemy.Task.Task.getMember(constructor_name);

		if (!constructor) {
			log.warn('Could not find task constructor:', constructor_name);
		}
	}
...
Service.setMethod(async function initSchedules() {
	...
	// Load the existing system schedules up front. This MUST NOT stream from
	// a live cursor: the loop below removes and saves records in the same
	// collection, and mutating a collection mid-cursor makes the cursor skip
	// documents - a skipped existing record then never lands in
	// `existing_system_records`, so the create loop below re-creates it as a
	// duplicate (a different one each boot, depending on cursor mechanics).
	let existing_records = await this.AlchemyTask.find('all', crit);

	// Iterate over all the existing system schedules in the database
	for (let record of existing_records) {
		...
		try {
			...
			// If the checksum is not in the required system schedules
			// (e.g. the task class no longer exists), remove it
			if (!entry || !entry.cron) {
				await record.remove();
				continue;
			}
	...
	for (let [checksum, entry] of required_system_schedules) {
		...
		try {
			...
			// Handle duplicate key errors gracefully - the record already exists
			if (err.code === 11000) {
				log.warning('Task with checksum', checksum, 'already exists, skipping creation');

				// Try to fetch the existing record
				try {
					let crit = this.AlchemyTask.find();
					crit.where('forced_schedule_checksum').equals(checksum);
					let existing = await this.AlchemyTask.find('first', crit);

					if (existing) {
						existing_system_records.set(checksum, existing);
					}
				} catch (fetch_err) {
					alchemy.registerError(fetch_err, {context: 'Failed to fetch existing task with checksum ' + checksum});
				}
			} else {
				alchemy.registerError(err);
			}
...
Service.setMethod(function getTaskConstructor(task_type_path) {
	...
	if (typeof task_type_path == 'string') {
		...
		if (!task_class) {
			log.warn('Could not find task constructor', task_type_path);
			return false;
		}
...
/**
 * Start a task manually from its document
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    1.4.0
 * @version  1.4.0
 *
 * @param    {Document.SystemTask}   task_document   The task document to run
 *
 * @return   {Pledge<Task>}
 */
Service.setMethod(async function startTaskManually(task_document) {

	if (!task_document) {
		throw new Error('No task document provided');
	}

	if (!task_document.type) {
		throw new Error('Task has no type configured');
	}

	// Get the task schedules instance for this task type
	let task_schedules = this.getTaskSchedules(task_document.type);

	if (!task_schedules) {
		throw new Error('Could not find task type: ' + task_document.type);
	}

	// Create a new TaskSchedule instance for this manual run
	// (Similar to how #startFromMenu works in TaskSchedules)
	let schedule = new TaskSchedule(task_schedules, null, task_document.settings, task_document);
...
class TaskSchedules {
	...
	async #startFromMenu(task_record) {
		...
		let task_instance = await alchemy.task_service.startTaskManually(task_record);
...
class TaskSchedule {
	...
	async getHistoryDocumentForDate(date) {
		...
		crit.where('system_task_id').equals(this.task_document.$pk);
		...
		if (!result) {
			...
			result.system_task_id = this.task_document?.$pk;
	...
	async #run(started_manually = false) {
		...
		try {
			if (started_manually || await this.ownsScheduledTask()) {
				...
				// Start the task (don't await yet - we need the ID first)
				let start_promise = instance.start();

				// Wait for the task to be initialized (has an ID)
				// This resolves once the history document is saved, before the executor runs
				...
				// Now await the rest of the task execution
				await start_promise;
			}
		} catch (err) {
			// Reject the pledge if it hasn't been resolved yet
			if (this.#instance_pledge) {
				this.#instance_pledge.reject(err);
				this.#instance_pledge = null;
			}
