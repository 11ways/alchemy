...
SocketConduit.setMethod(function onPayload(packet) {
	...
	if (Router.socketRoutes[packet.type]) {
		...
		if (packet.args) {
			...
			// Put the conduit on top
			args.unshift(this);

			// Add "respond" callback to the bottom
			args.push(respond);
		} else if (packet.stream) {
			args = [this, packet.data, packet.stream, respond];
		} else {
			args = [this, packet.data, respond];
		}
...
SocketConduit.setMethod(function submit(type, data, stream, callback) {
	...
	if (stream) {
		console.log('Emitting with stream', stream, packet);
		// Emit on the IOStream socket
		return this.stream.emit('payload', stream, packet);
	} else {
		console.log('Emitting without stream', packet);
	}
...
console.log('Registering linkup cb on', client, cb);
	if (cb) {
