...
Schema.setStatic(function isSchema(value) {
	...
	if (Classes.Alchemy.Schema && value instanceof Classes.Alchemy.Schema) {
		return true;
	}
...
Schema.setMethod(function addAssociation(relation_type, relation_config, alias, model_name, options) {
	...
	this.addAssociationFieldToSchema(alias, 'AssociationAlias', null, null, client_doc);
});

/**
 * Add a field to the current schema & also add it to the given document class
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    1.4.0
 * @version  1.4.0
 ...
 * @param    {string}   type
 * @param    {Object}   options
 * @param    {Function} server_doc
 * @param    {Function} client_doc
 */
Schema.setMethod(function addAssociationFieldToSchema(name, type, options, server_doc, client_doc) {

	// Actually add the field to the schema
	let field = this.addField(name, type, options),
	    method = 'setFieldGetter';

	if (field.is_meta_field && field instanceof Classes.Alchemy.Field.AssociationAlias) {
		method = 'setAliasGetter';
	}

	if (server_doc) {
		server_doc[method](name);
	}

	// Also add it to the client doc, but make sure it does not in its turn
	// try to add the field to the server doc.
	if (client_doc) {
		client_doc[method](name, null, null, false);
	}
 ...
 * @param    {Object}             options
 * @param    {Array}              options.fields   Array of field names for compound indexes
 *
 * @return   {FieldType}
 */
Schema.setMethod(function addIndex(_field_or_name, _options) {
	...
	// Support compound indexes with a `fields` array
	...
	let field = this.getField(_field_or_name);

	if (!field) {
		throw new Error('Could not find field "' + _field_or_name + '"');
	}
 ...
 * @return   {boolean}
 */
Schema.setMethod(function isFieldRequired(field_name) {

	// Check if the field was explicitly marked as required
	let field = this.get(field_name);

	if (field && field.options && field.options.required) {
		return true;
	}

	// Check the rules for a NotEmpty validator on this field
	for (let rule of this.rules) {
		// Check if this rule applies to this field
		if (rule.options && rule.options.fields) {
			if (rule.options.fields.includes(field_name)) {
				// Check if it's a not_empty rule
				if (rule.constructor.name === 'NotEmpty' || rule.constructor.type_name === 'not_empty') {
					return true;
				}
			}
		}
...
/**
 * Convert this schema to a JSON Schema representation.
 * This is useful for MCP tool definitions, API documentation,
 * and other tools that use JSON Schema.
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    1.4.0
 * @version  1.4.0
 ...
 * @param    {boolean}  options.include_private  Include private fields (default: false)
 * @param    {boolean}  options.include_meta     Include meta fields (default: false)
 * @param    {string}   options.option_prefix    Prefix to look for in field options (e.g., 'mcp_')
 *
 * @return   {Object}   A JSON Schema object describing this schema
 */
Schema.setMethod(function toJsonSchema(options) {
	...
	for (let i = 0; i < fields.length; i++) {
		...
		// Skip private fields by default
		if (field.is_private && !options.include_private) {
			continue;
		}

		// Skip meta fields by default (like association aliases)
		if (field.is_meta_field && !options.include_meta) {
			continue;
		}

		// Get the field's JSON Schema, passing options through
		let field_schema = field.toJsonSchema(options);

		if (field_schema) {
			properties[field.name] = field_schema;

			// Check if the field is required
			if (this.isFieldRequired(field.name)) {
	...
	// Add title from schema name or model name
	if (this.name) {
		schema.title = this.name;
	} else if (this.model_name) {
		schema.title = this.model_name;
	}

	// Add description from options if available
