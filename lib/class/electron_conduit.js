 ...
 */
var ElectronConduit = Function.inherits('Conduit', function ElectronConduit(request, callback, router) {
...
ElectronConduit.setMethod(function _end(message, encoding) {
	console.log('Should call end:', message);
	//return this.response.end(message, 'utf-8');
	this.response({mimeType: 'text/html', data: new Buffer(message)});
});