...
alchemy.create(function SocketClient() {
	...
	this.preInit = function preInit(socket) {
		...
		iostream(this.socket).on('fileTransfer', function(stream, packet) {
			...
			var output = fs.createWriteStream('/tmp/received');

			// If the client needs a response, listen to the end of the output stream
			if (packet.respond) {
				output.on('finish', function() {
					// Send a response to the client
					that.socket.emit('response', {respond_to: packet.id});
				});
			}
		...
		this.sendFile('/tmp/nothing', function() {
			pr('We sent the nothing file from server to client')
		});
	};
	 ...
	 * @param    {Function} callback
	 */
	this.sendFile = function sendFile(filepath, callback) {
		...
		iostream(this.socket).emit('fileTransfer', stream, {path: filepath, id: id, respond: respond});
