 ...
 * @return   {Object}
 */
PathDefinition.setMethod(function getParametersObject(values) {
...
PathDefinition.setMethod(function test(path, conduit) {
	...
	values.forEach(function eachValue(value, index) {
		...
		if (token.typedef) {
			if (PathDefinition.typedefs[token.typedef]) {
				type_check = PathDefinition.typedefs[token.typedef](value, token.name, conduit);
			} else {
				TypeClass = Object.path(Blast.Classes.Alchemy, token.typedef) || Object.path(Blast.Classes, token.typedef);

				if (TypeClass && TypeClass.checkPathValue) {
					type_check = TypeClass.checkPathValue(value, token.name, conduit);
				}
			}
