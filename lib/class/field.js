	...
	if (this.is_translatable && !done_translation) {
		...
		for (i = 0; i < wrapped.length; i++) {
			...
			for (key in entry) {
				...
				if (this.is_array) {
					// The value has to be wrapped first: `_getValue` treats
					// anything with a `length` as the array, so a primitive
					// translation string was iterated CHARACTER BY CHARACTER
					// (`{en: 'foo'}` became `{en: ['f','o','o']}`)
					target[key] = this._getValue(Array.cast(entry[key]), true);
				} else {
...
Field.setMethod(function checkIndexes() {
	...
	// Handle explicit index request (true or options object)
	if (this.options.index === true) {
		index.indexed = true;
	} else if (this.options.index && typeof this.options.index === 'object') {
		Object.assign(index, this.options.index);
	}

	// Add index if we have any index configuration and a schema
	if (!Object.isEmpty(index) && this.schema) {
...
/**
 * Datatype to JSON Schema type mapping
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    1.4.0
 * @version  1.4.0
...
/**
 * Get the JSON Schema type for this field's datatype
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    1.4.0
 * @version  1.4.0
 ...
 * @return   {string}
 */
Field.setMethod(function getJsonSchemaType() {
	return DATATYPE_TO_JSON_SCHEMA[this.datatype] || 'string';
});

/**
 * Hook method for subclasses to augment the JSON Schema
 * with field-specific properties (format, enum, etc.)
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    1.4.0
 * @version  1.4.0
 ...
 * @param    {Object}   options   Optional configuration (passed through from toJsonSchema)
 */
Field.setMethod(function augmentJsonSchema(schema, options) {
	// Base implementation does nothing
	// Subclasses can override to add format, enum, etc.
});

/**
 * Get a prefixed option value with fallback to the regular option.
 *
 * @author   Jelle De Loecker <jelle@elevenways.be>
 * @since    1.4.0
 * @version  1.4.0
 *
 * @param    {string}   option_name   The option name (e.g., 'description', 'title')
 * @param    {string}   prefix        The prefix to look for (e.g., 'mcp_')
 *
 * @return   {*}   The prefixed value if available, otherwise the regular value
 */
Field.setMethod(function getPrefixedOption(option_name, prefix) {

	if (prefix && this.options) {
		let prefixed_value = this.options[prefix + option_name];
...
/**
 * Convert this field to a JSON Schema representation.
 ...
 */
Field.setMethod(function toJsonSchema(options) {

	let option_prefix = options?.option_prefix;

	// Get the base type
	let type = this.getJsonSchemaType();
	...
	// Add title if available (check prefixed version first)
	...
	// Add description if available (check prefixed version first)
	...
	if (this.has_default) {
		let default_value = this.options.default;

		// Don't include function defaults - they're dynamic
		if (typeof default_value !== 'function') {
			schema.default = default_value;
		}
	}

	// Let subclasses add their specific properties
	this.augmentJsonSchema(schema, options);

	// Handle nullable - add null to the type
	// Note: is_nullable is true by default unless explicitly set to false
	// For arrays, nullable applies to items (whether array items can be null)
	if (this.is_nullable) {
	...
	// Handle arrays - wrap the schema in an array type
	if (this.is_array) {
		schema = {
			type: 'array',
			items: schema,
