...
Datasource.setMethod(function create(model, data, options, callback) {
	...
	// Normalize the data
	this.toDatasource(model, data, function gotDatasourceValue(err, ds_data) {
		...
		model.emit('to_datasource', data, ds_data, options, true, function afterTDSevent(err, stopped) {
			...
			that._create(model, ds_data, options, callback);
		});
	});
});
...
Datasource.setMethod(function read(model, query, options, callback) {
	...
	if (this.queryCache && model.cache) {
		...
		if (cacheResult != null) {
			cacheResult.hinder.push(function returnCache() {
	...
	// Prepare the function to do the actual query
	reader = function doRead(callback) {
		that._read(model, query, options, function afterRead(err, results, available) {
	...
	if (this.queryCache && model.cache) {
		// Create a cache object with only a hinder,
		// so the exact same query that is already in progress
		// doesn't execute twice
		cacheResult = {
			hinder: Function.hinder(function hinderedRead(done) {
				reader(function gotResult(err, results, available) {
					var cloned;
					...
					model.emit('storing_cache', options, results);

					// Do the actual cloning
					cloned = JSON.clone(results);

					// Store the cloned object in the cache
					cacheResult.items = cloned;
					cacheResult.available = available;

					// Emit the stored_cache event
					model.emit('stored_cache', options, results);

					// Do the first callback
					callback(null, results, available);

					// And tell the hinder we're done
					done();
				});
			})
		};

		// Store the empty (for now) object, with the hinder, in the cache
		model.cache.set(serialized, cacheResult);
	} else {
		reader(callback);
	}
});
...
Datasource.setMethod(function update(model, data, options, callback) {
	...
	this.toDatasource(model, data, function gotDatasourceValue(err, ds_data) {
		...
		model.emit('to_datasource', data, ds_data, options, false, function afterTDSevent(err, stopped) {
			...
			// Call the real _update method
			that._update(model, ds_data, options, callback);
		});
	});
});
