...
Datasource.setMethod(function toApp(context) {
	...
	for (let key in data) {
		if (key[0] == '_' && key[1] == '$') {
			// Extra bookkeeping data (offline-cache fields like
			// _$needs_remote_save) is stored as "_$" properties. Carry them
			// through unconditionally: the offline-sync machinery reads them
			// off revived records, and it must behave the same on the server
			// (where the fallback datasource is exercised in tests) as in
			// the browser. Rows without such keys are unaffected.
			tasks[key] = data[key];
		}
	}
