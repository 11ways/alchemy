 ...
 * @param    {Object}         data
 */
Datasource.setMethod(function toApp(schema, data, callback) {
	...
	Object.each(data, function eachField(value, fieldName) {
		...
		if (field != null) {
			tasks[fieldName] = function doToDatasource(next) {
				that.valueToApp(field, value, next);
			};
		}
	});
 ...
 * @version  1.0.0
 */
Datasource.setMethod(function valueToApp(field, value, callback) {

	var that = this;

	field.toApp(value, function gotToAppValue(err, value) {
		if (err) {
			return callback(err);
		}

		that._valueToApp(field, value, callback);
	});
});
 ...
 * @version  1.0.0
 */
Datasource.setMethod(function _valueToApp(field, value, callback) {
	setImmediate(function immediateDelay() {
		callback(null, value);
	});
});
...
Datasource.setMethod(function read(model, query, options, callback) {
	...
	if (this.queryCache && model.cache) {
		serialized = 'query-' + hash(JSON.dry({query: query, options: options}));
		...
		if (cacheResult != null) {
			setImmediate(function returnCache() {
				...
				result = JSON.undry(cacheResult.items);
	...
	this._read(model, query, options, function afterRead(err, results, available) {
		...
		Function.parallel(tasks, function done(err, app_results) {
			...
			if (serialized != null) {
				...
				dried = JSON.dry(app_results);

				model.cache.set(serialized, {items: dried, available: available});
			}
