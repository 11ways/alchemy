...
alchemy.sputnik.defineStage('hawkejsSetup', function() {
	...
	Router.use('/hawkejs/template', function(req, res) {
		...
		alchemy.hawkejs.getTemplatePath(name, function(err, path) {

			if (!path) {
				res.end(404, 'Could not find ' + req.body.name);
			} else {
				req.conduit.serveFile(path);
			}
		});
	}, {methods: ['post'], weight: 19});
...
alchemy.sputnik.defineStage('middleware', function() {
	...
	log.todo('Middleware: compress, jsonparser, urlencodedparser, multipart, cookies, sessions, toobusy');

	// Serve public files
	Router.use('/public/', alchemy.publicMiddleware, 50);

	// Serve stylesheets
	Router.use('/stylesheets/', alchemy.lessMiddleware, 50);
	...
	Router.use(function parseBody(req, res, next) {
		...
		// Regular form-encoded or JSON data is handled by "body"
		anyBody(req, function parsedBody(err, body) {
			...
			if (err) {
				log.error('Error parsing regular POST', {err: err});
				req.original.body = {};
			} else {
