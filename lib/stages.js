...
alchemy.sputnik.defineStage('hawkejsSetup', function() {
	...
	// We need to rewrite some stuff inside express (viewfile handling)
	//hawkejs.configureExpress(alchemy.app, express);

	// Make client code & templates available to the public
	alchemy.sputnik.after('routes', function() {

		// Template serving
		log.todo('Serve hawkejs templates');
		// alchemy.app.post('/hawkejs/template', function(req, res) {
		...
		// // Client file serving
		// alchemy.app.get('/hawkejs/hawkejs-client.js', function(req, res) {
...
alchemy.sputnik.defineStage('middleware', function() {
	...
	lessOptions = {
		...
		preprocess: {},
		postprocess: {},
		dest: path.resolve(PATH_TEMP, 'stylesheets', 'compiled')
	};

	// I don't know what happened to lessMiddleware, but suddenly it's quite retarded
	// in regards to how it handles files
	lessOptions.preprocess.path = function modifyLessPath(pathname, req) {
		return pathname.replace(PATH_TEMP + '/stylesheets/public' , PATH_TEMP);
	};

	if (settings.config.lessSourceMap) {
		lessOptions.compiler.sourceMap = true;
	}

	lessMiddle = lessmw(path.resolve(PATH_TEMP, 'stylesheets'), lessOptions);
	...
	// This is a fix needed for lessMiddleware's new handling of files. Very awkward
	alchemy.static('/', path.resolve(PATH_TEMP, 'stylesheets', 'compiled', 'public'));
	...
	alchemy.addMiddleware(800, 'less', function(req, res, next){
		// Less will see if it needs to compile something, but it won't serve anything itself!
...
alchemy.sputnik.defineStage('startServer', function() {
	alchemy.server.listen(settings.config.port, function(){
		...
		// Launch the 'listening' stage
		alchemy.sputnik.launch('listening');
	});
});

alchemy.sputnik.after(['datasources', 'static', 'socket'], function() {
...
alchemy.sputnik.after(['views', 'stylesheets', 'static'], function() {
