...
alchemy.sputnik.defineStage('middleware', function() {
	...
	alchemy.app.use(lessmw({
		src:   path.resolve(PATH_TEMP, 'less'), // Source folder
		paths: [], // Import paths
		dest:  path.resolve(PATH_TEMP, 'stylesheets') // Compiled folder
	}));
	...
	pr('Using ' + PATH_TEMP);
	alchemy.app.use('/public/', alchemy.express.static(PATH_TEMP));
	//die();
