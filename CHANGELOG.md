...
* Persist the executor's return value on the `System.TaskHistory` row (new `result` Mixed field, saved in `Task#start`'s terminal bookkeeping): without it a task run that wrote NOTHING was indistinguishable from one that did everything - the run record only said `done`. A non-cloneable result degrades to an explanatory string instead of breaking the history save
...
* Fix `GroupValue#rebuildLeaf` DROPPING an object config value on an object-TYPE leaf setting: when a config file / early `setSetting` supplies a nested object for a setting whose real (later-declared) definition is an object-holding LEAF (`type: 'object'`/`mixed`/`schema`), the orphan loader groupifies it, and rebuildLeaf then rejected the group-value on a leaf as malformed and discarded it (warning `"...is a leaf but received a group override value; ignoring it"`). This silently broke any app storing a credentials blob that way (Arcana's `google.credentials`, killing its Gmail + Calendar integrations). Such a value is now RECOVERED by collapsing the group back to a plain object via `toObject()`; a genuine SCALAR leaf receiving an object is still dropped
...
* Fix the associated-save cascade stamping the foreign key on the LIST object of a plural association (a no-op): new children of a populated hasMany were created WITHOUT their parent's foreign key, and every unchanged populated child was re-saved as a side effect. Children are now checked & stamped individually
* Fix the server `Document.unDry` reading `$_attributes` where `toDry` stores `$attributes` (and never restoring `$hold`): a dry/undry round-trip silently dropped the original_record and with it the document's entire dirty state. Also stop `setDataRecord` re-snapshotting the original_record over a restored one
* Fix `Model#eachRecord()` writing its pagination condition into the CALLER's conditions object (and clobbering any caller condition on the primary key from page 2 on): the condition is now AND-ed in on a copy
* Fix `Model#remove()` event handling: behaviour `beforeRemove`/`afterRemove` hooks were never invoked (only the model method), `.on('removed')` listeners went dead when 1.4.0 renamed the emitted event, and `has_remove_events` ignored behaviours entirely. The legacy `removed` event is emitted again alongside `afterRemove`
* Fix the client `Document#save()` writing `create: true` into the caller's shared options object: a reused options object forced CREATE semantics onto the next, unrelated save
* `Model.hasServerAction()` returns false instead of throwing when hawkejs (or its scene) isn't ready: it gets called from idle callbacks where the throw was uncaught and left pledges unsettled
* Guard all sessionStorage access in the document cache-refresh path: property access alone throws with blocked cookies, and writes can throw in private browsing - either aborted `unDry` mid-revive
* Fix the AQL parser dropping all but the LAST parenthesized sibling group: `(a = 1) and (b = 2)` compiled to just `b = 2`, silently widening user filters. Closed groups now fold into their parent immediately
* Fix `select('OwnModel.field')`: a dead `this.model` check in `Select#parse` made the own-model prefix create a bogus association, rejecting the whole find at compile time
* Fix the `or(value)`/`and(value)` shorthand dropping the association of the active expression: `where('Boss.name').equals(a).or(b)` compiled its second branch against the ROOT `name` field
* Fix `applyConditions` regex handling: a string `$regex` was RegExp-ESCAPED (turning every pattern into a literal-text match), and a `{$options, $regex}` object threw when key order put `$options` first
* Fix `FieldExpression#normalizeAssociationValues` (the association-condition fallback for datasources without join support): the association conditions were passed as an unrecognized find option (querying the WHOLE associated collection) and the resulting rows were read with the wrong shape (every foreign key came back `undefined`)
* Apply the field selection on the mongo aggregate path: any `select()` combined with an association-forced `$lookup` silently returned FULL documents
* Fix the Criteria async iterator clobbering a pre-set `skip` (always starting at 0) and iterating straight past a pre-set `limit`
* Fix HABTM condition casting: the save-path cast wrapped scalars in arrays, so `equals(id)`/`in([ids])` compiled to exact-array matches that never hit a multi-element field. Conditions now use mongo's containment semantics
* Fix `store_units` date fields dropping the conversion of every chained condition after the first: `.gte(a).lte(b)` compiled to `{$gte: <number>, $lte: <Date>}` and matched nothing
* Fix `Task#stop()` on a paused task being a permanent deadlock: the executor was parked on the pause pledge (which only `resume()` settles), so it could never observe the stop - and the task's cron schedule stopped repeating for the process lifetime. Also make `has_stopped` reflect a REQUESTED stop, since that property is what executors poll and nothing else ever exposed `need_stop` to them
* Fix `syncDataWithServer()` never settling (and leaking an unhandled rejection) when a drain errored: `Function.parallel` ignores an async task's returned promise, so a throw meant `next` was never called. Per-record replay failures now leave the record queued for the next attempt
* Fix a primitive translation value on a translatable+array field being iterated character by character on the document-setter path (`{en: 'foo'}` stored as `{en: ['f','o','o']}`) - the same 1.4.6 bug, on the other conversion side
* `Mongo#connect()` returns a rejected pledge instead of THROWING synchronously while a connection error is cached: connect() is evaluated eagerly as a waterfall argument, so the sync throw bypassed rejection handlers - a Fallback datasource stopped serving cached reads for the whole retry window
* Fix IndexedDB reads sorting in the wrong direction: the sort compiler passes an ARRAY of [path, direction] pairs, but the IDB result sorter consumed it as a {path: direction} map - ascending came back descending and multi-key directions shifted one key over
* Fix IndexedDB reads dropping the `available` count (paginated UIs got `undefined` totals when served from the offline cache)
* Evict rejected pledges from the IDB collection cache and the Fallback connect cache (both were plain memoizes): one transient IndexedDB error kept that table - or the whole datasource - broken for the session lifetime
* Cache what the remote actually SAVED after a successful online save, not the request payload: the server-bumped `updated` (and any server-side hook mutations) never reached the offline cache, so every later freshness comparison used a stale timestamp
* Implement `Fallback#_remove()`: fallback-datasource models could never delete anything (the inherited base `_remove` rejects unconditionally). Deletes go to the lower datasource and purge the cached upper copy best-effort
* Reject an offline-queued save when the lower datasource refused it with a VALIDATION error: it was indistinguishable from being offline, so the save resolved successfully and the doomed record was silently replayed forever
* Stop `storeInUpperDatasource` and `Remote#update` from mutating the caller's save-options object (`extraneous`/`create` leaked into unrelated saves)
* Reject a read combining `only_local` and `only_remote` with a clear error instead of a stack overflow (empty waterfall)
* Include the target datasource in the criteria checksum: a Fallback datasource reads the same criteria against both its sub-datasources, and the shared query cache could serve the LOWER's result for an upper (raw cache) read - resurrecting the unqueued-offline-save bug on server-side fallbacks with query caching
* Fix the criteria compiler attaching a nested `$or`/`$and` as a SIBLING of the other keys in a multi-key condition object: inside an `$or` branch, `{a: 1, $or: [...]}` compiled to `a OR ...` instead of `a AND (...)`, so a bounded range query silently matched an entire collection (Mimir's per-day statistics recompute fetched all 310k activities per day and OOM'd a 4GB heap). Such objects now compile via the equivalent single-key `{$and: [{a: 1}, {$or: [...]}]}`
* Fix the NoSQL compiler unwrapping a nested group's compiled `{$and: [...]}` and rewrapping it with the PARENT entry's operator: a single-item group collapses to its item, so an `$or` entry could receive AND-ed conditions and flip them to `$or` - `a AND b` became `a OR b`. The unwrap now only happens when the result gets rewrapped as `$and` again
* Make the offline-sync replay conflict-safe: `getRecordsToBeSavedRemotely()` now checks the remote version of every queued record first (via the new `only_remote` criteria option, which reads the lower datasource authoritatively instead of falling back to the cache). A remote updated AFTER the queued edit wins: the stale local copy is dropped and the remote version adopted into the cache (`Fallback#adoptRemoteVersion`) - a flag left behind by an old session can no longer overwrite newer server data. An unreachable remote defers the replay
* Fix `Document#informDatasource()` WIPING the pending-save flag whenever it re-cached a revived record: it read `_$needs_remote_save` off the Document (where `toApp` never put it), so merely READING an offline-edited record silently unqueued its save. It now consults the raw cached row (new `Fallback#getUpperVersion`) and preserves both the flag and the original local-save time - re-stamping "now" would have made stale edits beat newer remote versions in conflict resolution
* Carry the `_$` offline-cache bookkeeping fields through `Datasource#toApp` on the server too (they were browser-only), so the sync machinery behaves identically in server-side tests and in the browser
...
* Fix a stored `[null]` entry in an array field making the whole record unreadable: one null sub-schema entry (with associations) rejected the ENTIRE `find()`. Array-field entry loops now honor the documented `_toApp`/`_toDatasource` "no nullish entries" contract on both the read and write side, and `Field#getValue()` returns `[]` instead of `[null]` for a null array value (the source of those junk entries)
* Fix `Task#start()` terminal state: nothing ever set the STOPPED status (`has_stopped` stayed false forever), finished tasks were never removed from the shared `running` array (a pure memory leak for every cron run), the running pledge never settled on stop/error, and a failure before initialization (e.g. a transient DB error on the history-document save) left `waitUntilInitialized()` pending forever - which wedged the task schedule permanently
* Fix `Task.execute()` silently discarding its callback (it was passed as `start()`'s ignored second argument) and throwing when called without a callback for an unknown task name
* Fix `Migration#processRecords()` hanging the migration forever when the record handler throws (the rejection never reached the iterator's `next`), and propagate the completion error instead of discarding it
* Don't mark a COMPLETED migration as failed when saving its `ended` stamp fails: the success save now lives outside the try/catch that records migration errors (a failure there used to block all future migrations with a bogus error record)
* Fix `Model#eachRecord()` crashing on `result.length` instead of propagating a failed find
* Fix `Stage#_addTask()`'s late-add path installing a status pledge that nothing ever resolved: adding a task to an already-started type made `_launch` hang forever (e.g. a `SputnikShim.before()` on a running stage stalling the whole boot)
* Fix a failing stage task leaving the stage "started but never ended": pre/main/post task failures now reject the stage pledge (so `afterStages()` waiters and dependent stages see the error) instead of only surfacing to `_launch`'s caller - and the fire-and-forget parent `refreshStatus()` call no longer produces an unhandled rejection
* Fallback datasource: a rejected lower (remote) read no longer rejects the whole query - it falls through to the upper (local cache) read, so being offline serves cached data instead of failing every find
* Fallback datasource: `getRecordsToSync()` now unwraps the `{items, available}` shape `Datasource#read` returns since 1.4.2 - offline saves never synced because the consumer iterated the wrapper object
* Fallback datasource: an offline save no longer runs `toApp` on already-app-format data (the local store's result was converted twice, corrupting any field whose app and datasource formats differ), and when BOTH the remote and local save fail the actual save error is surfaced instead of a misleading "Unable to convert data: no data given"
* `Document#save()` keeps fields that were edited while the save was in flight instead of silently reverting them to the response, and marks them changed so a follow-up save persists them. Fields mutated by save hooks are excluded from that detection (they match the save result), so a normal save still ends clean
* Fix `Datasource#toApp`'s entity cache serving projection-truncated records: the cache keyed on (model, id) only, but the same record can appear with different per-alias `$project`ions in one read - a fully-populated alias could receive the truncated version cached by another alias. The key now includes the entry's field names
* Fix the mongo datasource permanently caching a rejected collection pledge: one failed connect at boot kept that table broken for the process lifetime, bypassing `connect()`'s own retry logic
* Only translate actual WRITE errors (duplicate key, document validation) into validation Violations in the mongo `_create`: matching on any `err.code` also reclassified infrastructure errors (auth failure, stepdown, shutdown) as user-input problems
* Evict a rejected read from the model's query cache instead of replaying the error for the full cache TTL
* Fix the Sluggable behaviour skipping a record's own available slug: the duplicate-check loop left the PREVIOUS iteration's found item in place when the found record was the record being saved, forcing an unnecessary counter bump
* Fix the Publishable behaviour hiding records whose `publish_date` is stored as an explicit `null`: the "no publish date" branch used `$exists: false`, which does not match a present-but-null field - it now uses the `isEmpty` compiler
* Harden the Revision behaviour: the original-data weakmap entry (keyed on the possibly-shared options object) is now cleared on a failed lookup and consumed in `afterSave`, so one document's old data can no longer be diffed against ANOTHER document saved with the same options object; lookup errors are registered instead of silently skipping the revision
* Fix `getDocumentClass()` caching under the wrong key (short class name instead of the full model name): namespaced models never hit the cache, and a root model sharing a namespaced model's base name (e.g. `Task` vs `System.Task`) could receive the wrong document class
* Fix two 1.4.0-refactor leftovers in `SchemaField` that crashed with a ReferenceError/TypeError instead of resolving: `getSubSchemaFromModel()` called `resolveSchemaPath()` with its pre-1.4.0 signature (and an undeclared `path` variable), and the partial-save recovery path passed a plain record where `getSubSchema()` expects an operational context. Also `auditRecord()` still called `datasource.read()` with its pre-1.4.0 signature, breaking alternate-index audits
* Fix `translatableToDatasource()` iterating a primitive value character-by-character (storing `{"0":"f","1":"o","2":"o"}` for `'foo'`): it now wraps primitives in the default prefix like its read twin does
* Fix `checkAndInformDatasource()` returning a pledge that never settled on the no-op paths, a misordered null guard in `toApp`'s field loop, an unguarded `options` access in `setDataRecord()`, an out-of-scope variable in a `SchemaField` warning, and remove a debug leftover (`bla: 1`) from every dried Field
* Stop persisting meta fields (like `AssociationAlias`) in `Datasource#toDatasource()`: loading a document whose sub-schema has associations attaches the populated alias (a full Document wrapper: `$options`, `$record`, even the model instance) to each entry at runtime, and the save path wrote that straight into the datasource. Besides bloating every record with a sub-schema `belongsTo`, a self-referencing object graph inside the wrapper made saves die with "Cannot convert circular structure to BSON" (this took down Arcana's live purchase-invoice sync). The `toApp` path already skipped meta fields; the save path now mirrors it
...
* Fix `PaginateComponent#find` storing an empty `PageInfo` object and only mutating it afterwards: the renderer's internal store now snapshots/transforms the value on set, so the later `PageInfo[name] = entry` mutation never reached the view and `Pagination.show()` rendered nothing but inactive arrows. Now populates the entry first and stores `PageInfo` once
